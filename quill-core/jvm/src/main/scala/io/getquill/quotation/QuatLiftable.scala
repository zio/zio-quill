...
trait QuatLiftable {
  ...
  implicit val quatProductLiftable: Liftable[Quat.Product] =
    Liftable[Quat.Product] {
      ...
      case quat: Quat.Product if (serializeQuats) =>
        q"io.getquill.quat.Quat.Product.fromSerializedJVM(${quat.serializeJVM})"
      case Quat.Product.WithRenamesCompact(
            tpe,
            fields,
            values,
            renamesFrom,
        ...
        q"io.getquill.quat.Quat.Product.WithRenamesCompact.apply($tpe)(..$fields)(..$values)(..$renamesFrom)(..$renamesTo)"
    }

  implicit val quatProductTypeLiftable: Liftable[Quat.Product.Type] =
    Liftable[Quat.Product.Type] {
      case Quat.Product.Type.Concrete =>
        q"io.getquill.quat.Quat.Product.Type.Concrete"
      case Quat.Product.Type.Abstract =>
        q"io.getquill.quat.Quat.Product.Type.Abstract"
    }

  implicit val quatLiftable: Liftable[Quat] = Liftable[Quat] {
    case quat: Quat.Product if (serializeQuats) =>
      q"io.getquill.quat.Quat.fromSerializedJVM(${quat.serializeJVM})"
    case Quat.Product.WithRenamesCompact(
          tpe,
          fields,
          values,
