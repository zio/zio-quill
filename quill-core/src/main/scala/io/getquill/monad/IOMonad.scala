...
trait IOMonad {
  ...
  object IO {
    ...
    def sequence[A, M[X] <: IterableOnce[X], E <: Effect](in: M[IO[A, E]])(
        implicit cbfResultToValue: Factory[A, M[A]]
    ): IO[M[A], E] =
    ...
    def failed[T](exception: Throwable): IO[T, Effect] = fromTry(
      Failure(exception)
    )
    ...
    def traverse[A, B, M[X] <: IterableOnce[X], E <: Effect](in: M[A])(
        fn: A => IO[B, E]
    )(implicit cbf: Factory[B, M[B]]): IO[M[B], E] =
