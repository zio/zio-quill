...
trait Parsing extends ValueComputation with QuatMaking {
  ...
  val queryParser: Parser[Ast] = Parser[Ast] {
    ...
    case q"$pack.querySchema[$t](${name: String}, ..$properties)" =>
      ...
      Entity.Opinionated(
        name,
        properties.map(propertyAliasParser(_)),
        inferQuat(q"$t".tpe).probit,
        Fixed
      )
  ...
  implicit val propertyAliasParser
      : Parser[PropertyAlias] = Parser[PropertyAlias] {
  ...
  val joinCallParser: Parser[(JoinType, Ast, Option[Ast])] =
    Parser[(JoinType, Ast, Option[Ast])] {
      case q"$a.join[$t, $u]($b)" if (is[DslQuery[Any]](a)) =>
        (InnerJoin, astParser(a), Some(astParser(b)))
      case q"$a.leftJoin[$t, $u]($b)" if (is[DslQuery[Any]](a)) =>
        (LeftJoin, astParser(a), Some(astParser(b)))
      case q"$a.rightJoin[$t, $u]($b)" if (is[DslQuery[Any]](a)) =>
        (RightJoin, astParser(a), Some(astParser(b)))
      case q"$a.fullJoin[$t, $u]($b)" if (is[DslQuery[Any]](a)) =>
        (FullJoin, astParser(a), Some(astParser(b)))

      case q"$a.join[$t]" if (is[DslQuery[Any]](a)) =>
        (InnerJoin, astParser(a), None)
      case q"$a.leftJoin[$t]" if (is[DslQuery[Any]](a)) =>
        (LeftJoin, astParser(a), None)
      case q"$a.rightJoin[$t]" if (is[DslQuery[Any]](a)) =>
        (RightJoin, astParser(a), None)
    }

  val infixParser: Parser[Ast] = Parser[Ast] {
    case q"$infix.pure.asCondition" =>
      Quat.improveInfixQuat(
    ...
    case q"$infix.transparent.pure.as[$t]" =>
      Quat.improveInfixQuat(
        combinedInfixParser(true, Quat.Generic, true)(infix)
      )
  ...
  val impureInfixParser = combinedInfixParser(
    false,
    Quat.Value
  ) // TODO Verify Quat in what cases does this come up?

  def combinedInfixParser(
      infixIsPure: Boolean,
      quat: Quat,
      infixIsTransparent: Boolean = false
    ...
    case q"$pack.InfixInterpolator(scala.StringContext.apply(..${parts: List[String]})).infix(..$params)" =>
      ...
      if (parts.find(_.endsWith("#")).isDefined) {
        val elements =
          parts.zipWithIndex.flatMap {
            case (part, idx) if idx < params.length =>
              if (part.endsWith("#")) {
                Left(
                  q"${part.dropRight(1)} + String.valueOf(${params(idx)})"
                ) :: Nil
              } else {
        ...
        val liftUnlift = new { override val mctx: c.type = c } with LiftUnlift(
          newParams.foldLeft(0)((num, ast) => num + ast.countQuatFields)
        )
        import liftUnlift._
  ...
  val identParser: Parser[Ident] = Parser[Ident] {
    ...
    case t: ValDef =>
      identClean(
        Ident(t.name.decodedName.toString, inferQuat(t.symbol.typeSignature))
      )
    case id @ c.universe.Ident(TermName(name)) =>
      identClean(Ident(name, inferQuat(id.symbol.typeSignature)))
    case t @ q"$cls.this.$i" =>
      identClean(
        Ident(i.decodedName.toString, inferQuat(t.symbol.typeSignature))
      )
    case t @ c.universe
  ...
  private def ident(x: TermName, quat: Quat): Ident = identClean(
    Ident(x.decodedName.toString, quat)
  )
  ...
  /** In order to guarentee consistent behavior across multiple databases, we
    ...
    * `Option.forall`, and `Option.exists` are used. However, we would like
    * users to be warned that the behavior of improperly structured queries may
    * change as a result of this modification (see #1302 for more details). This
    * method search the subtree of the respective Option methods and creates a
    * warning if any `If(_, _, _)` AST elements are found inside. Since these
    * can be found deeply nested in the AST (e.g. inside of `BinaryOperation`
    * nodes etc...) it is necessary to recursively traverse into the subtree via
    * a stateful transformer in order to discovery this.
    */
  ...
  /** Process scala Option-related DSL into AST constructs. In Option[T] if T is
    ...
    * "Unchecked."
    *
    * The `isOptionRowType` method checks if the type-parameter of an option is
    * a Product. The isOptionEmbedded checks if it an embedded object.
    */
  ...
  private def operationParser(cond: Tree => Boolean)(
    ...
    Parser[Operation] {
      case q"$a.${operator(op: BinaryOperator)}($b)"
          if cond(
            a
          ) => // test on right tree is not necessary and broken in 2.13
  ...
  case object Equal extends EqualityBehavior {
    def operator: BinaryOperator = EqualityOperator.`_==`
  }
  ...
  private def equalityWithInnerTypechecksIdiomatic(left: Tree, right: Tree)(
    ...
    (leftIsOptional, rightIsOptional, equalityBehavior) match {
      ...
      case (true, true, Equal) =>
        (OptionIsEmpty(a) +&&+ OptionIsEmpty(b)) +||+ (OptionIsDefined(
          a
        ) +&&+ OptionIsDefined(b) +&&+ comparison)
      ...
      case (true, true, NotEqual) =>
        (OptionIsDefined(a) +&&+ OptionIsEmpty(b)) +||+ (OptionIsEmpty(
          a
        ) +&&+ OptionIsDefined(b)) +||+ comparison
      ...
      case (lop, rop, _) => {
        ...
        val ropString =
          (if (rop) "Optional" else "Non-Optional") + s" ${right}}"
        c.abort(
          left.pos,
  ...
  /** In situations where the a `.returning` clause returns the initial record
    ...
    * fields i.e. `.returning(r => (r.foo, r.bar))` otherwise the tokenizer
    * would be force to pass `RETURNING *` to the SQL which is a problem because
    * the fields inside of the record could arrive out of order in the result
    * set (e.g. arrive as `r.bar, r.foo`). Use use the value/flatten methods in
    * order to expand the case-class out into fields.
    */
  private def reprocessReturnClause(
    ...
    (ident == originalBody, actionType.tpe.dealias) match {
      ...
      case (true, ClassTypeRefMatch(cls, List(arg)))
          if (cls == asClass[DslInsert[_]] || cls == asClass[
            DslUpdate[_]
          ] || cls == asClass[DslDelete[_]]) && isTypeCaseClass(arg) =>
        val elements =
  ...
  private val assignmentDualParser: Parser[AssignmentDual] =
    Parser[AssignmentDual] {
      case q"((${identParser(i1)}, ${identParser(
            i2
          )}) => $pack.Predef.ArrowAssoc[$t]($prop).$arrow[$v]($value))" =>
  ...
  private val assignmentParser: Parser[Assignment] = Parser[Assignment] {
    ...
    case astParser(ast) =>
      Assignment(
        Ident("unused", Quat.Value),
        Ident("unused", Quat.Value),
        Constant("unused", Quat.Value)
      )
  }
