...
trait Liftables extends QuatLiftable {
  ...
  implicit val astLiftable: Liftable[Ast] = Liftable[Ast] {
    ...
    case Infix(a, b, pure, transparent, quat) =>
      q"$pack.Infix($a, $b, $pure, $transparent, $quat)"
    case If(a, b, c) => q"$pack.If($a, $b, $c)"
    case Dynamic(tree: Tree, _)
        if (tree.tpe <:< mctx.weakTypeOf[Quoted[Any]]) =>
      q"$tree.ast"
  ...
  implicit val queryLiftable: Liftable[Query] = Liftable[Query] {
    case Entity.Opinionated(a, b, quat, renameable) =>
      q"$pack.Entity.Opinionated($a, $b, $quat, $renameable)"
  ...
  implicit val propertyAliasLiftable: Liftable[PropertyAlias] =
    Liftable[PropertyAlias] { case PropertyAlias(a, b) =>
  ...
  implicit val assignmentDualLiftable: Liftable[AssignmentDual] =
    Liftable[AssignmentDual] { case AssignmentDual(a1, a2, b, c) =>
      q"$pack.AssignmentDual($a1, $a2, $b, $c)"
    }

  implicit val valueLiftable: Liftable[Value] = Liftable[Value] {
    case NullValue => q"$pack.NullValue"
    case Constant(a, quat) =>
      q"$pack.Constant(${Literal(mctx.universe.Constant(a))}, $quat)"
  ...
  implicit val externalIdentLiftable: Liftable[ExternalIdent] =
    Liftable[ExternalIdent] { case ExternalIdent(a, quat) =>
      q"$pack.ExternalIdent($a, $quat)"
    }

  implicit val liftLiftable: Liftable[Lift] = Liftable[Lift] {
    case ScalarValueLift(a, b: Tree, c: Tree, quat: Quat) =>
      q"$pack.ScalarValueLift($a, $b, $c, $quat)"
    case CaseClassValueLift(a, b: Tree, quat: Quat) =>
      q"$pack.CaseClassValueLift($a, $b, $quat)"
    case ScalarQueryLift(a, b: Tree, c: Tree, quat: Quat) =>
      q"$pack.ScalarQueryLift($a, $b, $c, $quat)"
    case CaseClassQueryLift(a, b: Tree, quat: Quat) =>
      q"$pack.CaseClassQueryLift($a, $b, $quat)"
  }
