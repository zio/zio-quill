...
trait ReifyLiftings extends QuatMaking {
  ...
  private case class ReifyLiftings(state: Map[TermName, Reified])
    ...
    private def reify(lift: Lift) =
      lift match {
        case ScalarValueLift(name, value: Tree, encoder: Tree, _) =>
          Reified(value, Some(encoder))
        case CaseClassValueLift(name, value: Tree, _) => Reified(value, None)
        case ScalarQueryLift(name, value: Tree, encoder: Tree, _) =>
          Reified(value, Some(encoder))
        ...
        case QuotedReference(ref: Tree, refAst) =>
          ...
          val reparsedAst =
            (ref.tpe, refAst) match {
              case (
                    QuotedType(QueryType(tpe)),
                    i @ Infix(_, _, _, _, Quat.Placeholder(_))
                  ) =>
  ...
  protected def reifyLiftings(ast: Ast): (Ast, Tree) =
    ReifyLiftings(Map.empty)(ast) match {
      case (ast, _) =>
        ...
        val reduced =
          trace"ReifyLiftings BetaReduction: " andReturn BetaReduction(
            ast,
            TypeBehavior.ReplaceWithReduction
          )
        val retyped =
          trace"ReifyLiftings Retyped: " andReturn RepropagateQuats(reduced)
