...
trait Unliftables extends QuatUnliftable {
  ...
  implicit val astUnliftable: Unliftable[Ast] = Unliftable[Ast] {
    ...
    case q"$pack.Aggregation.apply(${a: AggregationOperator}, ${b: Ast})" =>
      Aggregation(a, b)
    case q"$pack.Infix.apply(${a: List[String]}, ${b: List[
          Ast
        ]}, ${pure: Boolean}, ${transparent: Boolean}, ${quat: Quat})" =>
  ...
  implicit val queryUnliftable: Unliftable[Query] = Unliftable[Query] {
    case q"$pack.Entity.apply(${a: String}, ${b: List[PropertyAlias]}, ${quat: Quat.Product})" =>
      Entity(a, b, quat)
    case q"$pack.Entity.Opinionated.apply(${a: String}, ${b: List[
          PropertyAlias
        ]}, ${quat: Quat.Product}, ${renameable: Renameable})" =>
  ...
  implicit def optionUnliftable[T](implicit
      u: Unliftable[T]
  ...
  implicit val conflictActionUnliftable: Unliftable[OnConflict.Action] =
    Unliftable[OnConflict.Action] {
      ...
      case q"$pack.OnConflict.Update.apply(${a: List[AssignmentDual]})" =>
        OnConflict.Update(a)
    }
  ...
  implicit val assignmentDualUnliftable
      : Unliftable[AssignmentDual] = Unliftable[AssignmentDual] {
    case q"$pack.AssignmentDual.apply(${a1: Ident}, ${a2: Ident}, ${b: Ast}, ${c: Ast})" =>
      AssignmentDual(a1, a2, b, c)
  }

  implicit val valueUnliftable: Unliftable[Value] = Unliftable[Value] {
    case q"$pack.NullValue" => NullValue
    case q"$pack.Constant.apply(${Literal(mctx.universe.Constant(a))}, ${quat: Quat})" =>
      Constant(a, quat)
  ...
  implicit val externalIdentUnliftable: Unliftable[ExternalIdent] =
    Unliftable[ExternalIdent] {
      case q"$pack.ExternalIdent.apply(${a: String}, ${quat: Quat})" =>
        ExternalIdent(a, quat)
    }

  implicit val liftUnliftable: Unliftable[Lift] = Unliftable[Lift] {
    case q"$pack.ScalarValueLift.apply(${a: String}, $b, $c, ${quat: Quat})" =>
      ScalarValueLift(a, b, c, quat)
    case q"$pack.CaseClassValueLift.apply(${a: String}, $b, ${quat: Quat})" =>
      CaseClassValueLift(a, b, quat)
    case q"$pack.ScalarQueryLift.apply(${a: String}, $b, $c, ${quat: Quat})" =>
      ScalarQueryLift(a, b, c, quat)
    case q"$pack.CaseClassQueryLift.apply(${a: String}, $b, ${quat: Quat})" =>
      CaseClassQueryLift(a, b, quat)
  }
