...
class BindMacroSpec extends Spec {
  ...
  "binds non-batched action" - {
    ...
    "case class lifting" in {
      ...
      val r = testContext.bind(q)
      r mustEqual Row("s", 1, 2L, None)
    }
    "nested case class lifting" in {
      val q = quote {
        (t: TestEntity) => qr1.insert(t)
      }
      val r = testContext.bind(q(lift(TestEntity("s", 1, 2L, None))))
      r mustEqual Row("s", 1, 2L, None)
    }
    "returning value" in {
      val q = quote {
  ...
  "binds batched action" - {
    ...
    "case class" in {
      ...
      val r = testContext.bind(q)
      r mustEqual List(Row("s1", 2, 3L, Some(4)), Row("s5", 6, 7L, Some(8)))
    }
    ...
    "zipWithIndex" in {
      ...
      val r = testContext.bind(q)
      r mustEqual List(Row(0, "s1", 2, 3, Some(4)), Row(1, "s5", 6, 7, Some(8)))
    }
    "scalar + returning" in {
      val insert = quote {
    ...
    "case class + returning" in {
      ...
      val r = testContext.bind(q)
      r mustEqual List(Row("s1", 2, Some(4)), Row("s5", 6, Some(8)))
    }
    "case class + returning + nested action" in {
      val insert = quote {
        (p: TestEntity) => qr1.insert(p).returning(t => t.l)
      }
      val r = testContext.bind(liftQuery(entities).foreach(p => insert(p)))
      r mustEqual List(Row("s1", 2, Some(4)), Row("s5", 6, Some(8)))
    }
  }
}
