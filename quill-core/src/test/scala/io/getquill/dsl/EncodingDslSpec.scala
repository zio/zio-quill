...
class EncodingDslSpec extends Spec {
  ...
  "materializes encoding for AnyVal" - {
    "encoder" in {
      ...
      enc(0, CustomValue(1), Row(), MirrorSession.default) mustEqual Row(1)
    }
    "decoder" in {
      val dec = implicitly[Decoder[CustomValue]]
      dec(0, Row(1), MirrorSession.default) mustEqual CustomValue(1)
    }
  }
  ...
  "materializes encoding for generic AnyVal" - {
    ...
    "decoder" in {
      ...
      dec(0, Row(1), MirrorSession.default) mustEqual CustomGenericValue(1)
    }
  }
