  ...
  "quotes and unquotes asts" - {
    ...
    "query" - {
      "schema" - {
        ...
        "without aliases" in {
          quote(unquote(qr1)).ast mustEqual Entity(
            "TestEntity",
            Nil,
            TestEntityQuat
          )
        ...
        "with property alias" in {
          ...
          quote(unquote(q)).ast mustEqual Entity.Opinionated(
            ...
            quatOf[TestEntity]
              .productOrFail()
              .renameAtPath(Nil, List("s" -> "theS", "i" -> "theI")),
            Fixed
          )
        }
        ...
        "explicit `Predef.ArrowAssoc`" in {
          ...
          val renamedQuat =
            TestEntityQuat.renameAtPath(Nil, List("s" -> "theS"))
          quote(unquote(q)).ast mustEqual Entity.Opinionated(
            "TestEntity",
        ...
        "with property alias and unicode arrow" in {
          ...
          val renamedQuat =
            TestEntityQuat.renameAtPath(Nil, List("s" -> "theS", "i" -> "theI"))
          quote(unquote(q)).ast mustEqual Entity.Opinionated(
            "SomeAlias",
            List(
              PropertyAlias(List("s"), "theS"),
        ...
        "with only some properties renamed" in {
          val q = quote {
            querySchema[TestEntity]("SomeAlias", _.s -> "theS").filter(t =>
              t.s == "s" && t.i == 1
            )
          }
          val renamedQuat =
          ...
          quote(unquote(q)).ast mustEqual (
            Filter(
              ...
              Ident("t", renamedQuat),
              (Property(Ident("t", renamedQuat), "s") +==+ Constant.auto(
                "s"
              )) +&&+ (Property(Ident("t", renamedQuat), "i") +==+ Constant
                .auto(1))
            )
          )
        }
        ...
        "with method and generic" in {
          def limitQuery[T] = quote { (q: Query[T]) =>
            infix"$q LIMIT 1".as[Query[T]]
          }
      ...
      "distinct" in {
        ...
        quote(unquote(q)).ast mustEqual Distinct(
          Entity("TestEntity", Nil, TestEntityQuat)
        )
      }
      "nested" in {
        ...
        quote(unquote(q)).ast mustEqual Nested(
          Entity("TestEntity", Nil, TestEntityQuat)
        )
      }
      "take" in {
    ...
    "action" - {
      ...
      "insert" - {
        ...
        "batch" in {
          ...
          quote(unquote(q)).ast mustEqual
            Foreach(
              ScalarQueryLift("q.list", list, intEncoder, QV),
              Ident("i"),
              delete.ast.body
            )
        }
        "batch with Quoted[Action[T]]" in {
          ...
          val insert = quote((row: ActionTestEntity) =>
            query[ActionTestEntity].insertValue(row)
          )
          val q = quote(liftQuery(list).foreach(row => quote(insert(row))))
          quote(unquote(q)).ast mustEqual
            Foreach(
              CaseClassQueryLift("q.list", list, quatOf[ActionTestEntity]),
              Ident("row"),
              insert.ast.body
            )
        }
      ...
      "delete" in {
        ...
        quote(unquote(q)).ast mustEqual Delete(
          Entity("TestEntity", Nil, TestEntityQuat)
        )
      }
      ...
      "tuple" - {
        "literal" in {
          ...
          quote(unquote(q)).ast mustEqual Tuple(
            List(Constant.auto(1), Constant.auto("a"))
          )
        }
        "arrow assoc" - {
          "unicode arrow" in {
          ...
          "normal arrow" in {
            ...
            quote(unquote(q)).ast mustEqual Tuple(
              List(
                Tuple(List(Constant.auto(1), Constant.auto("a"))),
                Constant.auto("b")
              )
            )
          }
          "explicit `Predef.ArrowAssoc`" in {
            ...
            quote(unquote(q)).ast mustEqual Tuple(
              List(Constant.auto("a"), Constant.auto("b"))
            )
          }
        }
      }
      ...
      "==" - {
        ...
        "comparing compatible nested types" - {
          ...
          "succeeds when Option/Option" in {
            ...
            quote(unquote(q)).ast.body mustEqual (OptionIsEmpty(
              ia
            ) +&&+ OptionIsEmpty(ib)) +||+ (OptionIsDefined(
              ia
        ...
        "extras" - {
          ...
          "succeeds when Option/Option" in {
            ...
            quote(unquote(q)).ast.body mustEqual OptionIsDefined(
              ia
            ) +&&+ OptionIsDefined(ib) +&&+ (ia +==+ ib)
          }
          "succeeds when Option/T" in {
      ...
      "!=" - {
        ...
        "comparing compatible nested types" - {
          ...
          "succeeds when Option/Option" in {
            ...
            quote(unquote(q)).ast.body mustEqual (OptionIsDefined(
              ia
            ) +&&+ OptionIsEmpty(ib)) +||+ (OptionIsEmpty(
              ia
            ) +&&+ OptionIsDefined(ib)) +||+ (ia +!=+ ib)
          }
    ...
    "dynamic" - {
      ...
      "type param" - {
        "simple" in {
          ...
          test[TestEntity].ast mustEqual Entity(
            "TestEntity",
            Nil,
            TestEntityQuat
          )
