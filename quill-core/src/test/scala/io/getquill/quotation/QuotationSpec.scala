  ...
  "quotes and unquotes asts" - {
    ...
    "query" - {
      "schema" - {
        ...
        "with embedded property alias" in {
          ...
          quote(unquote(q)).ast mustEqual Entity.Opinionated("SomeAlias",
                                                             List(PropertyAlias(List("ev", "s"), "theS"),
        ...
        "explicit `Predef.ArrowAssoc`" in {
          ...
          quote(unquote(q)).ast mustEqual Entity.Opinionated("TestEntity",
        ...
        "with only some properties renamed" in {
          ...
          quote(unquote(q)).ast mustEqual (
            Filter(
              ...
              Ident("t", renamedQuat),
              (Property(Ident("t", renamedQuat), "s") +==+ Constant.auto("s")) +&&+ (Property(Ident("t", renamedQuat),
                                                                                              "i"
              ) +==+ Constant.auto(1))
            )
          )
        ...
        "with implicit property and generic" in {
          ...
          q.ast mustEqual Infix(List("", " LIMIT 1"),
                                List(Entity("TableData", List(), Quat.LeafProduct("id"))),
                                false,
                                Quat.Generic
          )
          quote(unquote(q)).ast mustEqual Infix(List("", " LIMIT 1"),
                                                List(Entity("TableData", List(), Quat.LeafProduct("id"))),
                                                false,
                                                Quat.LeafProduct("id")
          )
      ...
      "withFilter" in {
        ...
        quote(unquote(q)).ast mustEqual Filter(Entity("TestEntity", Nil, TestEntityQuat),
                                               Ident("t"),
                                               BinaryOperation(Property(Ident("t"), "s"),
                                                               EqualityOperator.`==`,
                                                               Constant.auto("s")
                                               )
        )
      ...
      "map" in {
        ...
        quote(unquote(q)).ast mustEqual Map(Entity("TestEntity", Nil, TestEntityQuat),
                                            Ident("t"),
                                            Property(Ident("t"), "s")
        )
      }
      "flatMap" in {
        ...
        quote(unquote(q)).ast mustEqual FlatMap(Entity("TestEntity", Nil, TestEntityQuat),
                                                Ident("t", TestEntityQuat),
      ...
      "concatMap" in {
        ...
        quote(unquote(q)).ast mustEqual ConcatMap(Entity("TestEntity", Nil, TestEntityQuat),
                                                  Ident("t"),
                                                  BinaryOperation(Property(Ident("t", TestEntityQuat), "s"),
      ...
      "sortBy" - {
        "default ordering" in {
          ...
          quote(unquote(q)).ast mustEqual SortBy(Entity("TestEntity", Nil, TestEntityQuat),
                                                 Ident("t", TestEntityQuat),
                                                 Property(Ident("t"), "s"),
                                                 AscNullsFirst
          )
      ...
      "groupBy" in {
        ...
        quote(unquote(q)).ast mustEqual GroupBy(Entity("TestEntity", Nil, TestEntityQuat),
                                                Ident("t", TestEntityQuat),
      ...
      "aggregation" - {
        "min" in {
          ...
          quote(unquote(q)).ast mustEqual Aggregation(AggregationOperator.`min`,
                                                      Map(Entity("TestEntity", Nil, TestEntityQuat),
                                                          Ident("t", TestEntityQuat),
        ...
        "max" in {
          ...
          quote(unquote(q)).ast mustEqual Aggregation(AggregationOperator.`max`,
                                                      Map(Entity("TestEntity", Nil, TestEntityQuat),
                                                          Ident("t"),
                                                          Property(Ident("t", TestEntityQuat), "i")
                                                      )
        ...
        "avg" in {
          ...
          quote(unquote(q)).ast mustEqual Aggregation(AggregationOperator.`avg`,
                                                      Map(Entity("TestEntity", Nil, TestEntityQuat),
                                                          Ident("t"),
                                                          Property(Ident("t", TestEntityQuat), "i")
                                                      )
        ...
        "sum" in {
          ...
          quote(unquote(q)).ast mustEqual Aggregation(AggregationOperator.`sum`,
                                                      Map(Entity("TestEntity", Nil, TestEntityQuat),
                                                          Ident("t"),
                                                          Property(Ident("t", TestEntityQuat), "i")
                                                      )
        ...
        "size" in {
          ...
          quote(unquote(q)).ast mustEqual Aggregation(AggregationOperator.`size`,
                                                      Map(Entity("TestEntity", Nil, TestEntityQuat),
                                                          Ident("t"),
                                                          Property(Ident("t", TestEntityQuat), "i")
                                                      )
      ...
      "union" in {
        ...
        quote(unquote(q)).ast mustEqual Union(Entity("TestEntity", Nil, TestEntityQuat),
                                              Entity("TestEntity2", Nil, TestEntity2Quat)
        )
      }
      "unionAll" - {
        ...
        "++" in {
          ...
          quote(unquote(q)).ast mustEqual UnionAll(Entity("TestEntity", Nil, TestEntityQuat),
                                                   Entity("TestEntity2", Nil, TestEntity2Quat)
          )
        }
      }
    ...
    "binary operation" - {
        ...
        "string interpolation" - {
          ...
          "one param" - {
            ...
            "start" in {
              ...
              normStrConcat(quote(unquote(q)).ast.body) mustEqual BinaryOperation(Ident("i"),
                                                                                  StringOperator.`+`,
                                                                                  Constant.auto("v")
              )
            }
          ...
          "multiple params" in {
            ...
            normStrConcat(quote(unquote(q)).ast.body) mustEqual BinaryOperation(
              BinaryOperation(
                BinaryOperation(BinaryOperation(Ident("i"), StringOperator.`+`, Constant.auto("a")),
                                StringOperator.`+`,
                                Ident("j")
                ),
                StringOperator.`+`,
      ...
      "contains" - {
        ...
        "startsWith" in {
          ...
          quote(unquote(q)).ast.body mustEqual BinaryOperation(Ident("s"),
    ...
    "unary operation" - {
      ...
      "nonEmpty" in {
        ...
        quote(unquote(q)).ast mustEqual UnaryOperation(SetOperator.`nonEmpty`,
                                                       Entity("TestEntity", Nil, TestEntityQuat)
        )
      }
      "isEmpty" in {
    ...
    "option operation" - {
      ...
      "exists" - {
        ...
        "unchecked" in {
          ...
          quote(unquote(q)).ast.body mustEqual OptionTableExists(Ident("o"),
                                                                 Ident("v"),
    ...
    "if" - {
      ...
      "nested" in {
        ...
        quote(unquote(q)).ast.body mustEqual If(Ident("c1"),
                                                Constant.auto(1),
    ...
    "aggregates liftings of nested quotations" - {
      ...
      "with implicit class" - {
        ...
        "query" in {
          ...
          val l =
            q.liftings.`implicits.ToRadom(null.asInstanceOf[io.getquill.EntityQuery[io.getquill.testContext.TestEntity]]).toRandom.Implicits.this.random`
          l.value mustEqual 999
          l.encoder mustEqual intEncoder
        }
      }
