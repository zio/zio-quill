...
class StablizeLiftsSpec extends Spec {
  ...
  "stablize lifts" - {
    ...
    "ScalarQueryLift" in {
      ...
      val (stablized, state) = StablizeLifts.stablize(ast)
      stablized must matchPattern { case ScalarQueryLift("scalarQuery", StablizeLifts.Token(0), _, `astQuat`) =>
      }
      StablizeLifts.revert(stablized, state) mustEqual (ast)
    }
    ...
    "CaseClassQueryLift" in {
      ...
      val astQuat            = ast.quat
      val (stablized, state) = StablizeLifts.stablize(ast)
      stablized must matchPattern { case CaseClassQueryLift("caseClasses", StablizeLifts.Token(0), `astQuat`) =>
      }
    ...
    "multiple lifts" in {
      ...
      stablized must matchPattern {
        case BinaryOperation(
              ...
              StringOperator.`+`,
              ScalarValueLift("b", External.Source.Parser, StablizeLifts.Token(1), _, `quatB`)
            ) =>
      }
