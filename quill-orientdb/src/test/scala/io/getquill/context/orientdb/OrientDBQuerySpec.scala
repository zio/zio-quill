...
class OrientDBQuerySpec extends Spec {
  ...
  "tokenizers" - {
    "if" in {
      ...
      t.token(
        If(Ident("x"), Ident("b"), If(Ident("y"), Ident("b"), Ident("c")))
      ) mustBe stmt"if(x, b, c)"

    }
    "query" in {
      val t = implicitly[Tokenizer[AstQuery]]
      t.token(Entity("name", Nil, QEP)) mustBe SqlQuery(
        Entity("name", Nil, QEP)
      ).token
    }
    "sql query" in {
      ...
      intercept[IllegalStateException](
        t.token(e.copy(distinct = true)(Quat.Value))
      ).getMessage mustBe "OrientDB DISTINCT with multiple columns is not supported"

      val x = SelectValue(Ident("x"))
      intercept[IllegalStateException](
        t.token(e.copy(select = List(x, x), distinct = true)(Quat.Value))
      ).getMessage mustBe "OrientDB DISTINCT with multiple columns is not supported"

      val tb = TableContext(Entity("tb", Nil, QEP), "x1")
      t.token(
        e.copy(from = List(tb, tb))(Quat.Value)
      ) mustBe stmt"SELECT * FROM tb"

      val jn = FlatJoinContext(InnerJoin, tb.copy(alias = "x2"), Ident("x"))
      intercept[IllegalStateException](
        t.token(e.copy(from = List(tb, jn))(Quat.Value))
      )

      t.token(
        e.copy(limit = Some(Ident("1", Quat.Value)), offset = Some(Ident("2")))(
          Quat.Value
        )
      ) mustBe stmt"SELECT * SKIP 2 LIMIT 1"
      t.token(
        e.copy(limit = Some(Ident("1", Quat.Value)))(Quat.Value)
      ) mustBe stmt"SELECT * LIMIT 1"
      t.token(
        e.copy(offset = Some(Ident("2", Quat.Value)))(Quat.Value)
      ) mustBe stmt"SELECT * SKIP 2"
    ...
    "operation" in {
      ...
      t.token(
        UnaryOperation(StringOperator.`toUpperCase`, i)
      ) mustBe stmt"toUpperCase() (i)"
      t.token(
        UnaryOperation(StringOperator.`toLowerCase`, i)
      ) mustBe stmt"toLowerCase() (i)"
      intercept[IllegalStateException](
        t.token(UnaryOperation(BooleanOperator.`!`, i))
      )

      t.token(
        BinaryOperation(NullValue, EqualityOperator.`_==`, i)
      ) mustBe stmt"i IS NULL"
      t.token(
        BinaryOperation(i, EqualityOperator.`_!=`, NullValue)
      ) mustBe stmt"i IS NOT NULL"
      t.token(
        BinaryOperation(NullValue, EqualityOperator.`_!=`, i)
      ) mustBe stmt"i IS NOT NULL"
      t.token(BinaryOperation(i, NumericOperator.`+`, i)) mustBe stmt"i + i"
      intercept[IllegalStateException](
        t.token(BinaryOperation(i, EqualityOperator.`_!=`, i))
      )
    ...
    "select value" in {
      ...
      t.token(
        SelectValue(
          Aggregation(AggregationOperator.`max`, Entity("t", Nil, QEP)),
          Some("x")
        )
    ...
    "action" in {
      ...
      def ins(a: String) =
        Insert(
          Entity("tb", Nil, QEP),
          List(Assignment(i, Property(Property(i, "x"), a), i))
        )
      t.token(ins("isEmpty")) mustBe stmt"INSERT INTO tb (x IS NULL) VALUES(i)"
      t.token(
        ins("isDefined")
      ) mustBe stmt"INSERT INTO tb (x IS NOT NULL) VALUES(i)"
      t.token(
        ins("nonEmpty")
      ) mustBe stmt"INSERT INTO tb (x IS NOT NULL) VALUES(i)"
      t.token(
