...
trait CollectionDecoders {
  ...
  implicit def listDecoder[T]: Decoder[List[T]] = decoder((index, row, _) => {
    row.field[java.util.List[T]](row.fieldNames()(index)).asScala.toList
  })
  implicit def setDecoder[T]: Decoder[Set[T]] = decoder((index, row, _) => {
    row.field[OTrackedSet[T]](row.fieldNames()(index)).asScala.toSet
  })
  implicit def mapDecoder[K, V]: Decoder[Map[K, V]] = decoder((index, row, _) => {
