...
trait Decoders extends CollectionDecoders {
  ...
  case class OrientDBDecoder[T](decoder: BaseDecoder[T])
  ...
  def decoder[T](d: BaseDecoder[T]): Decoder[T] =
    OrientDBDecoder((index, row, session) =>
      if (
        index >= row.fieldNames().length || row.fieldValues()(index) == null
      ) {
  ...
  implicit val bigDecimalDecoder: Decoder[BigDecimal] =
    decoder((index, row, session) =>
      row.field[java.math.BigDecimal](row.fieldNames()(index))
    )
  implicit val booleanDecoder: Decoder[Boolean] =
    decoder((index, row, session) =>
      row.field[Boolean](row.fieldNames()(index))
    )
  implicit val intDecoder: Decoder[Int] =
    decoder((index, row, session) => row.field[Int](row.fieldNames()(index)))
  ...
  implicit val byteDecoder: Decoder[Byte] =
    decoder((index, row, session) => row.field[Byte](row.fieldNames()(index)))
  ...
  implicit val dateDecoder: Decoder[Date] =
    decoder((index, row, session) => row.field[Date](row.fieldNames()(index)))
}
