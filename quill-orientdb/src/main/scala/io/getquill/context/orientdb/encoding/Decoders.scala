...
trait Decoders extends CollectionDecoders {
  ...
  implicit val stringDecoder: Decoder[String] = decoder((index, row, _) => {
    row.field[String](row.fieldNames()(index))
  })
  implicit val doubleDecoder: Decoder[Double] = decoder((index, row, _) => row.field[Double](row.fieldNames()(index)))
  implicit val bigDecimalDecoder: Decoder[BigDecimal] = decoder((index, row, _) => row.field[java.math.BigDecimal](row.fieldNames()(index)))
  implicit val booleanDecoder: Decoder[Boolean] = decoder((index, row, _) => row.field[Boolean](row.fieldNames()(index)))
  implicit val intDecoder: Decoder[Int] = decoder((index, row, _) => row.field[Int](row.fieldNames()(index)))
  implicit val shortDecoder: Decoder[Short] = decoder((index, row, _) => row.field[Short](row.fieldNames()(index)))
  ...
  implicit val floatDecoder: Decoder[Float] = decoder((index, row, _) => row.field[Float](row.fieldNames()(index)))
  implicit val byteArrayDecoder: Decoder[Array[Byte]] = decoder((index, row, _) => {
