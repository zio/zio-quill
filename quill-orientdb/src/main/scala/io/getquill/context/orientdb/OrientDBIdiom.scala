...
trait OrientDBIdiom extends Idiom {
  ...
  override def emptySetContainsToken(field: Token): Token = StringToken(
    s"$field IS NULL"
  )
    ...
    val token =
      normalizedAst match {
        case q: Query =>
          ...
          val cleaned =
            if (!Messages.alwaysAlias) RemoveExtraAlias(naming)(refined)
            else refined
  ...
  implicit def ifTokenizer(implicit strategy: NamingStrategy): Tokenizer[If] =
  ...
  implicit def queryTokenizer(implicit
      strategy: NamingStrategy
  ): Tokenizer[Query] = Tokenizer[Query] { case q =>
    SqlQuery(q).token
  }
  ...
  implicit def orientDBQueryTokenizer(implicit
    ...
    case FlattenSqlQuery(
      ...
      val selectClause =
        select match {
          case Nil =>
            if (!distinct) stmt"SELECT *"
            else
              fail("OrientDB DISTINCT with multiple columns is not supported")
          case _ =>
            if (!distinct) stmt"SELECT ${select.token}"
            else if (select.size == 1)
              stmt"SELECT $distinctTokenizer(${select.token})"
            else
              fail("OrientDB DISTINCT with multiple columns is not supported")
        }
      ...
      (limit, offset) match {
        ...
        case (Some(limit), Some(offset)) =>
          stmt"$withOrderBy SKIP ${offset.token} LIMIT ${limit.token}"
        case (None, Some(offset)) => stmt"$withOrderBy SKIP ${offset.token}"
      }
    case SetOperationSqlQuery(a, op, b) =>
      val str =
        f"SELECT $$c LET $$a = (${a.token}), $$b = (${b.token}), $$c = UNIONALL($$a, $$b)"
      str.token
    case _ =>
      fail(
        "Other operators are not supported yet. Please raise a ticket to support more operations"
      )
  }
  ...
  protected def tokenOrderBy(criterias: List[OrderByCriteria])(implicit
      strategy: NamingStrategy
  ) =
  ...
  implicit def orderByCriteriaTokenizer(implicit
    ...
    case OrderByCriteria(ast, Asc | AscNullsFirst | AscNullsLast) =>
      stmt"${scopedTokenizer(ast)} ASC"
    case OrderByCriteria(ast, Desc | DescNullsFirst | DescNullsLast) =>
  ...
  implicit def selectValueTokenizer(implicit
    ...
    Tokenizer[SelectValue] {
      case SelectValue(ast, Some(alias), false) =>
        stmt"${tokenValue(ast)} ${strategy.column(alias).token}"
      case SelectValue(Ident("?", _), None, false) => "?".token
      case SelectValue(ast: Ident, None, false) =>
        stmt"*" //stmt"${tokenValue(ast)}.*"
      case SelectValue(ast, None, false) => tokenValue(ast)
      case SelectValue(_, _, true) =>
        fail("OrientDB doesn't support `concatMap`")
    }
  }
  ...
  protected def scopedTokenizer[A <: Ast](
      ast: A
  )(implicit token: Tokenizer[A]) =
    ast match {
