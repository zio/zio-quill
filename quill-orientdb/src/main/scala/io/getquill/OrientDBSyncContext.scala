...
class OrientDBSyncContext[N <: NamingStrategy](
  ...
  def this(naming: N, config: OrientDBContextConfig) =
    this(naming: N, config.dbUrl, config.username, config.password)
  def this(naming: N, config: Config) =
    this(naming: N, OrientDBContextConfig(config))
  def this(naming: N, configPrefix: String) =
    this(naming: N, LoadConfig(configPrefix))
  ...
  override def performIO[T](
    ...
    if (transactional)
      logger.underlying.warn(
        "OrientDB doesn't support transactions, ignoring `io.transactional`"
      )
    super.performIO(io)
  }
  ...
  def executeAction(
    ...
    oDatabase.command(
      orientQl,
      objects.toIndexedSeq.asInstanceOf[Seq[Object]]: _*
    )
    ()
  }

  def executeBatchAction[T](
