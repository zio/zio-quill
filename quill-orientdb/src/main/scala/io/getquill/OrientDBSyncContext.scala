...
class OrientDBSyncContext[N <: NamingStrategy](
  ...
  def executeQuery[T](orientQl: String,
    ...
    oDatabase
      .query[java.util.List[ODocument]](new OSQLSynchQuery[ODocument](checkInFilter(orientQl, objects.size)),
                                        objects.asJava
      )
      .asScala
      .map(extractor)
  ...
  def executeQuerySingle[T](orientQl: String,
                            prepare: Prepare = identityPrepare,
                            extractor: Extractor[T] = identityExtractor
  ): T =
