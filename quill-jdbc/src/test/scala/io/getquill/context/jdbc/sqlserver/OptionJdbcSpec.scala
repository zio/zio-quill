...
class OptionJdbcSpec extends OptionQuerySpec {
  ...
  val `Simple Map with GetOrElse Infix` = quote {
    query[Address].map(a =>
      (
        a.street,
        a.otherExtraInfo
          .map(info => infix"${info} + ' suffix'".as[String])
          .getOrElse("baz")
      )
    )
  }
  ...
  "Example 1 - Simple Map with Condition" in {
    testContext.run(
      `Simple Map with Condition`
    ) should contain theSameElementsAs `Simple Map with Condition Result`
  }
  ...
  "Example 2 - Simple GetOrElse" in {
    testContext.run(
      `Simple GetOrElse`
    ) should contain theSameElementsAs `Simple GetOrElse Result`
  }

  "Example 3 - LeftJoin with FlatMap" in {
    testContext.run(
      `LeftJoin with FlatMap`
    ) should contain theSameElementsAs `LeftJoin with FlatMap Result`
  }

  "Example 4 - LeftJoin with Flatten" in {
    testContext.run(
      `LeftJoin with Flatten`
    ) should contain theSameElementsAs `LeftJoin with Flatten Result`
  }

  "Example 5 - Map+getOrElse Join" in {
    testContext.run(
      `Map+getOrElse LeftJoin`
    ) should contain theSameElementsAs `Map+getOrElse LeftJoin Result`
  }

  "Example 6 - Map+Option+Flatten+getOrElse Join" in {
    testContext.run(
      `Option+Some+None Normalize`
    ) should contain theSameElementsAs `Option+Some+None Normalize Result`
  }
}
