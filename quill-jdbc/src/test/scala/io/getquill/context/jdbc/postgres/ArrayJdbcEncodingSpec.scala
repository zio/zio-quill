...
class ArrayJdbcEncodingSpec extends ArrayEncodingBaseSpec {
  ...
  val corrected =
    e.copy(timestamps = e.timestamps.map(d => new Timestamp(d.getTime)))
  ...
  "Custom decoders/encoders" in {
    ...
    implicit def arrayUUIDEncoder[Col <: Seq[UUID]]: Encoder[Col] =
      arrayRawEncoder[UUID, Col]("uuid")
    implicit def arrayUUIDDecoder[Col <: Seq[UUID]](implicit
        bf: CBF[UUID, Col]
    ): Decoder[Col] = arrayRawDecoder[UUID, Col]
  ...
  "empty array on found null" in {
    ...
    ctx
      .run(querySchema[E]("ArraysTestEntity"))
      .headOption
      .map(_.texts) mustBe Some(Nil)
  }
