...
class JdbcContextSpec extends Spec {
  ...
  "Insert with returning generated with multiple columns and query embedded" in {
    val inserted = ctx.run {
      qr4Emb
        .insertValue(lift(TestEntity4Emb(EmbSingle(0))))
  ...
  "update returning" - {
    ...
    "with multiple columns and query embedded" in {
      ctx.run(qr1Emb.delete)
      ctx.run(
        qr1Emb.insertValue(lift(TestEntityEmb(Emb("one", 1), 42L, Some(456))))
      )
  ...
  "delete returning" - {
    ...
    "with multiple columns and query embedded" in {
      ctx.run(qr1Emb.delete)
      ctx.run(
        qr1Emb.insertValue(lift(TestEntityEmb(Emb("one", 1), 42L, Some(333))))
      )
