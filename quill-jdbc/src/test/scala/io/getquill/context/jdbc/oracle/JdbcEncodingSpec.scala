...
class JdbcEncodingSpec extends EncodingSpec {
  ...
  "encodes sets" in {
    ...
    testContext.run(
      liftQuery(insertValues).foreach(p =>
        query[EncodingTestEntity].insertValue(p)
      )
    )
  ...
  def emptyAsNull(e: EncodingTestType) =
    e.copy(value = if (e.value == "") null else e.value)
  def emptyAsNull(str: String) = if (str == "") null else str
  def emptyAsNull(bytes: Array[Byte]) =
