...
class JdbcContextSpec extends Spec {
  ...
  "Insert with returning generated with single column table using query" in {
    ctx.run(qr5.delete)
    val id = ctx.run(
      qr5.insertValue(lift(TestEntity5(0, "foo"))).returningGenerated(_.i)
    )
    val id2 = ctx.run {
      qr5
        .insert(_.s -> "bar")
        .returningGenerated(r =>
          query[TestEntity5].filter(_.s == "foo").map(_.i).max
        )
    }.get
    id mustBe id2
  }
  ...
  "insert returning" - {
        ...
        qr1
          ...
          .returning(r =>
            (
              r.i,
              r.s + "_s",
              qr2.filter(rr => rr.i == r.i && rr.s == lift(value)).map(_.s).max
            )
          )
      }
      (36, "two_s", Some("foobar")) mustBe inserted
        ...
        qr1
          ...
          .returning(r =>
            (
              r.i,
              r.s + "_s",
              qr1.filter(rr => rr.o.exists(_ == r.i - 1)).map(_.s).max
            )
          )
      }
    ...
    "with multiple columns and query embedded" in {
      ...
      ctx.run(
        qr1Emb.insertValue(lift(TestEntityEmb(Emb("one", 1), 18L, Some(123))))
      )
      val inserted = ctx.run {
        qr1Emb
          .insertValue(lift(TestEntityEmb(Emb("two", 2), 18L, Some(123))))
          .returning(r => (r.emb.i, r.o))
      }
  ...
  "update returning" - {
    ...
    "with multiple columns and query embedded" in {
      ...
      val updated = ctx.run {
        qr1Emb
          .updateValue(lift(TestEntityEmb(Emb("two", 2), 18L, Some(123))))
          .returning(r => (r.emb.i, r.o))
      }
