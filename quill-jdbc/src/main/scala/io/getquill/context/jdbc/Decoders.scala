...
trait Decoders {
  ...
  implicit val dateDecoder: Decoder[util.Date] =
    decoder((index, row, _) => new util.Date(row.getTimestamp(index, Calendar.getInstance(dateTimeZone)).getTime))
}
...
trait BasicTimeDecoders { self: Decoders =>
  ...
  implicit val localDateDecoder: Decoder[LocalDate] =
    decoder((index, row, _) => row.getDate(index).toLocalDate)
  implicit val localTimeDecoder: Decoder[LocalTime] =
    decoder((index, row, _) => row.getTime(index).toLocalTime)
  ...
  implicit val zonedDateTimeDecoder: Decoder[ZonedDateTime] =
    decoder((index, row, _) => ZonedDateTime.ofInstant(row.getTimestamp(index).toInstant, dateTimeZone.toZoneId))
  implicit val instantDecoder: Decoder[Instant] =
    decoder((index, row, _) => row.getTimestamp(index).toInstant)
  ...
  implicit val offsetDateTimeDecoder: Decoder[OffsetDateTime] =
    decoder((index, row, _) => OffsetDateTime.ofInstant(row.getTimestamp(index).toInstant, dateTimeZone.toZoneId))
}

trait ObjectGenericTimeDecoders { self: Decoders =>
  implicit val localDateDecoder: Decoder[LocalDate] =
    decoder((index, row, _) => row.getObject(index, classOf[LocalDate]))
  implicit val localTimeDecoder: Decoder[LocalTime] =
    decoder((index, row, _) => row.getObject(index, classOf[LocalTime]))
  implicit val localDateTimeDecoder: Decoder[LocalDateTime] =
    decoder((index, row, _) => row.getObject(index, classOf[LocalDateTime]))

  implicit val zonedDateTimeDecoder: Decoder[ZonedDateTime] =
    decoder((index, row, _) => row.getObject(index, classOf[OffsetDateTime]).toZonedDateTime)
  implicit val instantDecoder: Decoder[Instant] =
    decoder((index, row, _) => row.getObject(index, classOf[OffsetDateTime]).toInstant)

  implicit val offsetTimeDecoder: Decoder[OffsetTime] =
    decoder((index, row, _) => row.getObject(index, classOf[OffsetTime]))
  implicit val offsetDateTimeDecoder: Decoder[OffsetDateTime] =
