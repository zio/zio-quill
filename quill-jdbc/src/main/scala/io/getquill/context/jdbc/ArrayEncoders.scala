...
trait ArrayEncoders extends ArrayEncoding {
  ...
  implicit def arrayStringEncoder[Col <: Seq[String]]: Encoder[Col] =
    arrayRawEncoder[String, Col](VARCHAR)
  ...
  /** Generic encoder for JDBC arrays.
    ...
    *   driver
    * @param mapper
    *   jdbc array accepts AnyRef objects hence a mapper is needed. If input
    *   type of an element of collection is not comfortable with jdbcType then
    *   use this mapper to transform to appropriate type before casting to
    *   AnyRef
    * @tparam T
    *   element type
    * @tparam Col
  ...
  /** Transform jdbcType int using `parseJdbcType` and calls overloaded method
    * to create Encoder
    *
    * @param jdbcType
    *   java.sql.Types
