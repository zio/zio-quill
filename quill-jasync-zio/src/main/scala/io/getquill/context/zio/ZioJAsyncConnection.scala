...
trait ZioJAsyncConnection {
  ...
  private[zio] final def transaction[A](
      action: RIO[Has[ZioJAsyncConnection], A]
  ): ZIO[Has[ZioJAsyncConnection], Throwable, A] = {
    ...
    takeConnection.use(conn =>
      (ZioJAsyncConnection.sendQuery(conn, "BEGIN") *>
        action.updateService[ZioJAsyncConnection](_ =>
          ZioJAsyncConnection.make(conn)
        )).tapBoth(
  ...
  private[zio] final def sendPreparedStatement(
      sql: String,
      params: Seq[Any]
  ): Task[QueryResult] =
    takeConnection.use(conn =>
...
object ZioJAsyncConnection {
  ...
  def sendQuery(
      query: String
  ): ZIO[Has[ZioJAsyncConnection], Throwable, QueryResult] =
  ...
  def sendPreparedStatement(
    ...
    ZIO.accessM[Has[ZioJAsyncConnection]](
      _.get.sendPreparedStatement(sql, params)
    )

  private def sendQuery[C <: ConcreteConnection](
      connection: C,
      query: String
  ): Task[QueryResult] =
    ZIO.fromCompletableFuture(connection.sendQuery(query))

  def make[C <: ConcreteConnection](
      pool: KConnectionPool[C]
  ): ZioJAsyncConnection = new ZioJAsyncConnection {

    override protected def takeConnection: TaskManaged[ConcreteConnection] =
      ZManaged.make(ZIO.fromCompletableFuture(pool.take()))(conn =>
        ZIO.fromCompletableFuture(pool.giveBack(conn)).orDie.unit
      )

  }

  def make[C <: ConcreteConnection](connection: C): ZioJAsyncConnection =
  ...
  def live[C <: ConcreteConnection: Tag]: ZLayer[Has[
    JAsyncContextConfig[C]
  ], Throwable, Has[ZioJAsyncConnection]] =
    ZManaged
      .accessManaged[Has[JAsyncContextConfig[C]]](env =>
        ZManaged.make(
          ZIO.effect(
            new KConnectionPool[C](
              env.get.connectionFactory(
                env.get.connectionPoolConfiguration.getConnectionConfiguration
              ),
      ...
      .map(ZioJAsyncConnection.make[C])
      .toLayer
