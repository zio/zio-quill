...
abstract class ZioJAsyncContext[
    D <: SqlIdiom,
    N <: NamingStrategy,
    C <: ConcreteConnection
](val idiom: D, val naming: N)
    extends Context[D, N]
  ...
  protected def extractActionResult[O](
  ...
  ): RIO[Has[ZioJAsyncConnection], List[T]] = {
  ...
  def executeBatchAction(groups: List[BatchGroup])(
      info: ExecutionInfo,
      dc: DatasourceContext
  ): RIO[Has[ZioJAsyncConnection], List[Long]] =
    ZIO
      .foreach(groups) { group =>
        ZIO
          .foldLeft(group.prepare)(List.newBuilder[Long]) {
            case (acc, prepare) =>
