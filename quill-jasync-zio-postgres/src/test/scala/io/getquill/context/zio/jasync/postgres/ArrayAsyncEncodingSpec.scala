...
class ArrayAsyncEncodingSpec extends ArrayEncodingBaseSpec with ZioSpec {
  ...
  "Joda times" in {
    case class JodaTimes(
        timestamps: Seq[JodaLocalDateTime],
        dates: Seq[JodaLocalDate]
    )
  ...
  "Catch invalid decoders" in {
    val newCtx = new TestContext {
      ...
      override implicit def arrayLocalDateDecoder[Col <: Seq[LocalDate]](
          implicit bf: CBF[LocalDate, Col]
      ): Decoder[Col] =
  ...
  "Arrays in where clause" in {
    runSyncUnsafe(context.run(q.insertValue(lift(e))))
    val actual1 =
      runSyncUnsafe(context.run(q.filter(_.texts == lift(List("test")))))
    val actual2 =
      runSyncUnsafe(context.run(q.filter(_.texts == lift(List("test2")))))
  ...
  "fail if found not an array" in {
    ...
    intercept[FiberFailure](
      runSyncUnsafe(context.run(query[EncodingTestEntity]))
    )
  }
