...
trait ZioSpec extends Spec with BeforeAndAfterAll {
  ...
  def accumulate[T](
      stream: ZStream[Has[ZioJAsyncConnection], Throwable, T]
  ): ZIO[Has[ZioJAsyncConnection], Throwable, List[T]] =
  ...
  def collect[T](
    ...
    Runtime.default.unsafeRun(
      stream.run(Sink.collectAll).map(_.toList).provideLayer(testContext.layer)
    )
  ...
  implicit class ZioTestExt[T](
      qzio: ZIO[Has[ZioJAsyncConnection], Throwable, T]
  ) {
