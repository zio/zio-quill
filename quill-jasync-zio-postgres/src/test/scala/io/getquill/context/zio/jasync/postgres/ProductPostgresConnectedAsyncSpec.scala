...
class ProductPostgresConnectedAsyncSpec extends ProductSpec with ZioSpec {
  import context._
  ...
  "Product" - {
    "Insert multiple products" in {
      ...
      val inserted = runSyncUnsafeWithConnected(ctx => {
        import ctx._
        ZIO.foreach(productEntries)(product =>
          ctx.run(productInsert(lift(product))))
      })

      val product = runSyncUnsafeWithConnected(ctx => {
        import ctx._
        ctx.run(productById(lift(inserted(2))))
      }).head
    ...
    "Single insert product" in {
      val inserted = runSyncUnsafeWithConnected(ctx => {
        import ctx._
        ctx.run(productSingleInsert)
      })
      val product = runSyncUnsafeWithConnected(ctx => {
        import ctx._
        ctx.run(productById(lift(inserted)))
      }).head
    ...
    "Single insert with inlined free variable" in {
      ...
      val returnedProduct = runSyncUnsafeWithConnected(ctx => {
        import ctx._
        ctx.run(productById(lift(inserted)))
      }).head
    ...
    "Single product insert with a method quotation" in {
      ...
      val inserted = runSyncUnsafeWithConnected(ctx => {
        import ctx._
        ctx.run(productInsert(lift(prd)))
      })
      val returnedProduct = runSyncUnsafeWithConnected(ctx => {
        import ctx._
        ctx.run(productById(lift(inserted)))
    ...
    "Single insert with value class" in {
      ...
      runSyncUnsafeWithConnected(ctx => {
        import ctx._
        val q1 = quote {
    ...
    "supports casts from string to number" - {
      "toInt" in {
        ...
        val queried = runSyncUnsafeWithConnected { ctx =>
          import ctx._
          ctx.run {
