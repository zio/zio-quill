...
abstract class JAsyncContextConfig[C <: ConcreteConnection](
  ...
  private def getString(path: String) =
    getValue(path, config.getString).toOption
  ...
  private lazy val urlConfiguration: Configuration =
    getValue("url", config.getString)
  ...
  lazy val connectionPoolConfiguration = new ConnectionPoolConfiguration(
    ...
    getLong("connectionValidationInterval").getOrElse(
      default.getConnectionValidationInterval
    ),
    getLong("connectionCreateTimeout").getOrElse(
      default.getConnectionCreateTimeout
    ),
    getLong("connectionTestTimeout").getOrElse(
      default.getConnectionTestTimeout
    ),
    ...
    Try(Charset.forName(config.getString("charset")))
      .getOrElse(urlConfiguration.getCharset),
    getInt("maximumMessageSize").getOrElse(
      urlConfiguration.getMaximumMessageSize
    ),
    urlConfiguration.getAllocator,
    getString("applicationName")
      .orElse(Option(urlConfiguration.getApplicationName))
      .orNull,
    urlConfiguration.getInterceptors,
    getLong("maxConnectionTtl")
      .map(JavaLong.valueOf)
      .orElse(Option(default.getMaxConnectionTtl))
      .orNull
  )
