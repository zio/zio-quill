...
class PostgresJAsyncContext[N <: NamingStrategy](
    naming: N,
    pool: ConnectionPool[PostgreSQLConnection]
) extends JAsyncContext[PostgresDialect, N, PostgreSQLConnection](
      PostgresDialect,
      naming,
      pool
    )
  ...
  def this(naming: N, config: Config) =
    this(naming, PostgresJAsyncContextConfig(config))
  ...
  override protected def extractActionResult[O](
    ...
    result.getRows.asScala.headOption
