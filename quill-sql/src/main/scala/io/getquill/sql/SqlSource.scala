...
abstract class SqlSource[R: ClassTag, S: ClassTag] extends io.getquill.impl.Source[R, S] {
  ...
  def run[P1, T](q: P1 => Actionable[T])(p1: P1): Any = macro SqlActionMacro.run1[P1, R, S, T]
  def run[P1, P2, T](q: (P1, P2) => Actionable[T])(p1: P1, p2: P2): Any = macro SqlActionMacro.run2[P1, P2, R, S, T]
}
