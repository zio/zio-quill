...
class QueryMacro(val c: Context) extends Quotation with SelectFlattening with SelectResultExtraction {
  ...
  private def runParametrized[R, S, T](query: Tree, bindings: List[Expr[Any]])(implicit r: WeakTypeTag[R], s: WeakTypeTag[S], t: WeakTypeTag[T]): Tree =
    query match {
      case q"(..$params) => $body" =>
  ...
  private def bindingMap(params: List[ValDef], bindings: List[Expr[Any]]) = {
    (for ((param, binding) <- params.zip(bindings)) yield {
      identUnliftable(param) -> (param, binding.tree)
    }).toMap
  }
