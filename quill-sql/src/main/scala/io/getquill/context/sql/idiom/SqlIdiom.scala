...
import io.getquill.idiom.{ Token, _ }
...
trait SqlIdiom extends Idiom {
      ...
      case r @ ReturningAction(Insert(table: Entity, Nil), alias, prop) =>
        idiomReturningCapability match {
          ...
          case OutputClauseSupported =>
            stmt"INSERT INTO ${table.token} OUTPUT INSERTED.* ${defaultAutoGeneratedToken(prop.token)}"
          case other =>
      ...
      case r @ ReturningAction(action, alias, prop) =>
        idiomReturningCapability match {
          ...
          case OutputClauseSupported => action match {
            case Insert(entity: Entity, assignments) =>
              val (table, columns, values) = insertInfo(insertEntityTokenizer, entity, assignments)
              stmt"INSERT $table${actionAlias.map(alias => stmt" AS ${alias.token}").getOrElse(stmt"")} (${columns.mkStmt(",")}) OUTPUT ${outputs(prop)} VALUES (${values.map(scopedTokenizer(_)).mkStmt(", ")})"
            case other =>
              fail(s"Action ast can't be translated to sql: '$other'")
          }
          case _ => stmt"${action.token}"
        }
  ...
  private def outputs(ast: Ast)(implicit tokenizer: Tokenizer[Ast]): Token = ast match {
    case property: Property                  => StringToken(s"INSERTED.${property.name}")
    case Tuple(values)                       => values.map(outputs).mkStmt()
    case CaseClass(values)                   => values.map(v => outputs(v._2)).mkStmt()
    case BinaryOperation(a: Property, op, b) => stmt"${outputs(a)} ${op.token} ${scopedTokenizer(b)}"
    case other                               => fail(s"Action ast can't be translated to sql: '$other'")
  }
