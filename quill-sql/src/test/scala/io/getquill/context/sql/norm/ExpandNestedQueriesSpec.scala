...
class ExpandNestedQueriesSpec extends Spec {
  ...
  "expands nested mapped entity correctly" in {
    ...
    val q = quote {
      qr1
        ...
        .on((a, b) => a.i == b.i)
        .nested
        .map(both => both match { case (a, b) => Dual(a, b) })
        .nested
    }
  ...
  "multiple embedding levels - another example - with rename" in {
    ...
    implicit val bimSchemaMeta =
      schemaMeta[Bim]("theBim", _.bid -> "theBid", _.mam.sim.sid -> "theSid")
  ...
  "expression subquery" - {
    ...
    "should be handled correctly in a regular schema" in {
      testContext
        .run(
          query[ThePerson].filter(p =>
            query[TheBoss].filter(_.bossId == p.bossId).map(_ => 1).nonEmpty
          )
        )
        .string mustEqual
    ...
    "should be handled correctly when using a schemameta" in {
      implicit val personSchema =
        schemaMeta[TheBoss]("theBossMan", _.bossId -> "bossman_id")
      testContext
        .run(
          query[ThePerson].filter(p =>
            query[TheBoss].filter(_.bossId == p.bossId).map(_ => 1).nonEmpty
