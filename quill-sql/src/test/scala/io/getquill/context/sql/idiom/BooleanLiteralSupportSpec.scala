...
class BooleanLiteralSupportSpec extends Spec {
  ...
  "value-fy boolean expression where needed" - testContext.withDialect(
    MirrorSqlDialectWithBooleanLiterals
  ) { ctx =>
    import ctx._
  ...
  "infix" - testContext.withDialect(MirrorSqlDialectWithBooleanLiterals) {
    ctx =>
      import ctx._
      ...
      "expressify asCondition" - {
        ...
        "distinct map-clause" in {
          val q = quote {
            query[Ent]
              .map(e => ("foo", infix"${e.i} > 123".asCondition))
              .distinct
              .map(r => ("baz", r._2))
          }.dynamic
          println(io.getquill.util.Messages.qprint(q.ast))
          ctx.run(q).string mustEqual
            "SELECT 'baz' AS _1, e._2 FROM (SELECT DISTINCT e._1, e._2 FROM (SELECT 'foo' AS _1, CASE WHEN e.i > 123 THEN 1 ELSE 0 END AS _2 FROM Ent e) AS e) AS e"
        }
        ...
        "pure distinct map-clause" in {
          val q = quote {
            query[Ent]
              .map(e => ("foo", infix"${e.i} > 123".pure.asCondition))
              .distinct
              .map(r => ("baz", r._2))
          }
          println(io.getquill.util.Messages.qprint(q.ast))
  ...
  "do not expressify string transforming operations" - {
    ...
    "first parameter" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
    ...
    "second parameter" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
    ...
    "both parameters" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
  ...
  "options" - {
    "exists" in testContext.withDialect(MirrorSqlDialectWithBooleanLiterals) {
      ctx =>
        import ctx._
        val q = quote {
          qr1
            .filter(t => t.o.exists(_ => if (false) false else true))
            .map(t => (t.b, true))
        }
        ctx.run(q).string mustEqual
    ...
    "exists - lifted" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
        qr1
          .filter(t =>
            t.o.exists(_ => if (lift(false)) lift(false) else lift(true))
          )
          .map(t => (t.b, true))
      }
      ctx.run(q).string mustEqual
  ...
  "joins" - {
    ...
    "join + map" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
    ...
    "join + map (with conditional)" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
        qr1
          .leftJoin(qr2)
          .on((a, b) => true)
          .map(t => (t._1.i, if (t._2.map(_.i > 20) === true) false else true))
      }
      ctx.run(q).string mustEqual
    ...
    "join + map (with conditional comparison)" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
        qr1
          .leftJoin(qr2)
          .on((a, b) => true)
          .map(t => (t._1.i, if (t._2.forall(_.i > 20)) false else true))
      }
      ctx.run(q).string mustEqual
        "SELECT a.i AS _1, CASE WHEN b IS NULL OR b.i > 20 THEN 0 ELSE 1 END AS _2 FROM TestEntity a LEFT JOIN TestEntity2 b ON 1 = 1"
    }

    "join + map (with conditional comparison lifted)" in testContext
      .withDialect(MirrorSqlDialectWithBooleanLiterals) { ctx =>
        import ctx._
        val q = quote {
          qr1
            .leftJoin(qr2)
            .on((a, b) => true)
            .map(t =>
              (t._1.i, if (t._2.forall(_.i > 20)) lift(false) else lift(true))
            )
        }
        ctx.run(q).string mustEqual
    ...
    "join + map + filter" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
    ...
    "join + map + filter (lifted)" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
    ...
    "for-comprehension with constant" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
    ...
    "for-comprehension with field" in testContext.withDialect(
      MirrorSqlDialectWithBooleanLiterals
    ) { ctx =>
      import ctx._
      val q = quote {
  ...
  "unary operators" - {
    "constant" in testContext.withDialect(MirrorSqlDialectWithBooleanLiterals) {
      ctx =>
        import ctx._
        val q = quote {
    ...
    "field" in testContext.withDialect(MirrorSqlDialectWithBooleanLiterals) {
      ctx =>
        import ctx._
        val q = quote {
