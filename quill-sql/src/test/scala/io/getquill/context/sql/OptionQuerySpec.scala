...
trait OptionQuerySpec extends Spec {
  ...
  val `Simple Map with Condition` = quote {
    query[Address].map(a =>
      (
        a.street,
        a.otherExtraInfo.map(info => if (info == "something") "one" else "two")
      )
    )
  }
  ...
  val `Simple Map with GetOrElse` = quote {
    query[Address].map(a =>
      (
        a.street,
        a.otherExtraInfo.map(info => info + " suffix").getOrElse("baz")
      )
    )
  }
  ...
  val `Simple Map with Condition and GetOrElse` = quote {
    query[Address].map(a =>
      (
        a.street,
        a.otherExtraInfo
          .map(info => if (info == "something") "foo" else "bar")
          .getOrElse("baz")
      )
    )
  }
  ...
  val `Simple GetOrElse` = quote {
    query[Address].map(a =>
      (a.street, a.otherExtraInfo.getOrElse("yet something else"))
    )
  }
  ...
  val `LeftJoin with FlatMap` = quote {
    query[Contact]
      ...
      .on((c, a) => c.addressFk.exists(_ == a.id))
  ...
  val `Map+getOrElse LeftJoin` = quote {
    query[Contact]
      .leftJoin(query[Address])
      .on((c, a) => c.addressFk.getOrElse(-1) == a.id)
      .map({ case (c, a) => (a.map(_.id), a.flatMap(_.otherExtraInfo)) })
  }
  ...
  val `Option+Some+None Normalize` = quote {
    val c1 = querySchema[NoAddressContact]("Contact").map(c =>
      (c.firstName, None: Option[Int])
    )
    val c2 = querySchema[HasAddressContact]("Contact").map(c =>
      (c.firstName, Some(c.addressFk))
    )
    val c3 = query[Contact].map(c => (c.firstName, c.addressFk))

    val normalized = (c1 ++ c2 ++ c3).map({ case (name, address) =>
      (name, normalizeAddress(address))
    })
