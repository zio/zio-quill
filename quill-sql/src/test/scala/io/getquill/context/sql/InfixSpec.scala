...
class InfixSpec extends Spec { //hello
  ...
  "queries with infix should" - {
    ...
    "preserve nesting where needed" in {
      val q = quote {
        query[Data]
          .map(e => TwoValue(e.id, infix"RAND()".as[Int]))
          .filter(r => r.value > 10)
      }
    ...
    "collapse nesting where not needed" in {
      val q = quote {
        query[Data]
          .map(e => TwoValue(e.id, infix"SOMETHINGPURE()".pure.as[Int]))
          .filter(r => r.value > 10)
      }
    ...
    "excluded infix values" - {
      ...
      "should not be dropped" in {
        val q = quote {
          query[Person]
            .map(p => (p.name, p.id, infix"foo(${p.other})".as[Int]))
            .map(p => (p._1, p._2))
        }
      ...
      "should not be dropped if pure" in {
        val q = quote {
          query[Person]
            .map(p => (p.name, p.id, infix"foo(${p.other})".pure.as[Int]))
            .map(p => (p._1, p._2))
        }

        ctx
          .run(q)
          .string mustEqual "SELECT p.name AS _1, p.id AS _2 FROM Person p"
      }
        ...
        val q = quote {
          query[Person]
            .map(p =>
              (
                p.name,
                (p.id, infix"foo(${p.other}, ${p.other2})".as[Int], p.other)
              )
            )
      ...
      "distinct-on infix example" in {
        val q = quote {
          query[Person]
            .map(p => (infix"DISTINCT ON (${p.other})".as[Int], p.name, p.id))
            .map(t => (t._2, t._3))
        }
