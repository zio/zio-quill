...
class OptionSqlSpec extends Spec {
  ...
  "Should correctly express optional rows to SQL" - {
    ...
    "flatten" in {
      testContext.run {
        query[Someone]
          .map(q => Some(q))
          .map(os => os.map(s => s.name.map(n => n + " Bloggs")).flatten)
      }.string mustEqual
    ...
    "flatMap" in {
      testContext.run {
        query[Someone]
          .map(q => Some(q))
          .map(os => os.flatMap(s => s.name.map(n => n + " Bloggs")))
      }.string mustEqual
        "SELECT q.name || ' Bloggs' FROM Someone q"
    }
  }
  "Should correctly express optional tables to SQL" - {
    "table flatMap" in {
      val q = quote {
        query[Someone]
          .map(q => Some(q))
          .flatMap(os =>
            query[Someone].join(s1 => s1.name == os.flatMap(oss => oss.name))
          )
      }
    ...
    "table foreach" in {
      testContext.run {
        query[Node].map(n => Some(n)).map(n => n.exists(nn => nn.uptime > 123))
      }.string mustEqual
    ...
    "liftable" - {
      "in map clause" - {
        "normally" in {
          testContext.run {
            query[Node].map(s =>
              s.isUp.map(n => n == true).getOrElse(lift(false))
            )
  ...
  "getOrElse with Booleans Literal Expansion" - testContext.withDialect(
