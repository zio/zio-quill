...
class OracleDialectSpec extends Spec {
  ...
  "quotes underscore columns and table aliases" - {
    "table and column select" in {
      ctx
        .run(query[_UnderscoreEntity].map(_t => (_t._1, _t._2)))
        .string mustEqual
    ...
    "table and column insert" in {
      ctx
        .run(
          query[_UnderscoreEntity]
            .insertValue(lift(_UnderscoreEntity("foo", 1, 1)))
        )
        .string mustEqual
  ...
  "uses dual for scalars" - {
    ...
    "Multi Scalar Select with Infix" in {
      ctx
        .run("foo" + infix"""'bar'""".as[String])
        .string mustEqual "SELECT 'foo' || 'bar' FROM DUAL"
    }
  }
