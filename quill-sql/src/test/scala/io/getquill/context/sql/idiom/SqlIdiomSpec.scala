...
class SqlIdiomSpec extends SqlSpec {

  "shows the sql representation of normalized asts" - {
    "query" - {
      "without filter" in {
        mirrorContext.run(qr1).sql mustEqual
      ...
      "unary operation" - {
        "nonEmpty" in {
          mirrorContext.run(qr1.nonEmpty).sql mustEqual
            "SELECT x.* FROM (SELECT EXISTS (SELECT x.* FROM TestEntity x)) x"
        }
        "isEmpty" in {
          mirrorContext.run(qr1.isEmpty).sql mustEqual
        ...
        "contains" - {
          ...
          "collection" - {
            "as value" in {
              ...
              mirrorContext.run(q1).sql mustEqual expectedSql
              mirrorContext.run(q2).sql mustEqual expectedSql
              mirrorContext.run(q3).sql mustEqual expectedSql
            }
            "as param" in {
              def verify[T](mirror: mirrorContext.QueryMirror[_], param: T) = {
              ...
              verify(mirrorContext.run(q1)(List(1, 2)), List(1, 2))
              verify(mirrorContext.run(q2)(Seq(1, 2)), Seq(1, 2))
              verify(mirrorContext.run(q3)(Set(1, 2)), Set(1, 2))
            }
          }
        }
      }
    ...
    "action" - {
      "insert" - {
        ...
        "generated" in {
          ...
          val run = mirrorContext.run(q)(List(TestEntity("s", 1, 2L, Some(1)))).sql mustEqual
    ...
    "infix" - {
      ...
      "full infix query" in {
        mirrorContext.run(infix"SELECT * FROM TestEntity".as[Query[TestEntity]]).sql mustEqual
          "SELECT x.s, x.i, x.l, x.o FROM (SELECT * FROM TestEntity) x"
      }
      "full infix action" in {
        mirrorContext.run(infix"DELETE FROM TestEntity".as[Action[TestEntity]]).sql mustEqual
          "DELETE FROM TestEntity"
      }
    }
    "if" - {
