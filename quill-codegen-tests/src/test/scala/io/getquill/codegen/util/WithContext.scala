...
trait WithContext[Prefix <: ConfigPrefix, CTest <: CodegenTestCases]
    extends SchemaMaker {
...
abstract class WithContextBase[
    Prefix <: ConfigPrefix,
    CTest <: CodegenTestCases
](val dbPrefix: Prefix, val ctest: CTest)
...
trait WithContextAux {
  type Aux[Prefix <: ConfigPrefix, CTest <: CodegenTestCases, Ret] =
    WithContext[Prefix, CTest] { type QuillContext = Ret }
