...
trait SqlIdiom extends Idiom {
  ...
  implicit def ifTokenizer(implicit astTokenizer: Tokenizer[Ast], strategy: NamingStrategy): Tokenizer[If] =
    Tokenizer[If] { case ast: If =>
      ...
      val (l, e)     = flatten(ast)
      val conditions =
        for ((cond, body) <- l) yield stmt"WHEN ${cond.token} THEN ${body.token}"
  ...
  implicit def sqlQueryTokenizer(implicit astTokenizer: Tokenizer[Ast], strategy: NamingStrategy): Tokenizer[SqlQuery] =
    Tokenizer[SqlQuery] {
  ...
  implicit def propertyTokenizer(implicit
    ...
    def tokenizePrefixedProperty(name: String,
                                 prefix: List[String],
                                 strategy: NamingStrategy,
      ...
      prefixRenameable match {
        case Renameable.Fixed =>
          (tokenizeColumn(strategy, prefix.mkString, prefixRenameable) + "." + tokenizeColumn(strategy,
                                                                                              name,
                                                                                              renameable
          )).token
        case _                =>
      ...
      case Property.Opinionated(ast, name, renameable, _ /* Top level property cannot be invisible */ ) =>
        ...
        unnest(ast) match {
          case (ExternalIdent.Opinionated(_: String, _, prefixRenameable), prefix) =>
            stmt"${actionAlias.map(alias => stmt"${scopedTokenizer(alias)}.").getOrElse(stmt"")}${tokenizePrefixedProperty(name, prefix, strategy, renameable, prefixRenameable)}"
          ...
          case (ExternalIdent(_, _), prefix)                                       =>
            stmt"${actionAlias.map(alias => stmt"${scopedTokenizer(alias)}.").getOrElse(stmt"")}${tokenizePrefixedProperty(name, prefix, strategy, renameable)}"
  ...
  implicit def infixTokenizer(implicit astTokenizer: Tokenizer[Ast], strategy: NamingStrategy): Tokenizer[Infix] =
    Tokenizer[Infix] { case Infix(parts, params, _, _) =>
  ...
  def returnListTokenizer(implicit tokenizer: Tokenizer[Ast], strategy: NamingStrategy): Tokenizer[List[Ast]] = {
    val customAstTokenizer =
      Tokenizer.withFallback[Ast](SqlIdiom.this.astTokenizer(_, strategy)) { case sq: Query =>
  ...
  )(implicit astTokenizer: Tokenizer[Ast], strategy: NamingStrategy): Tokenizer[Action] =
    Tokenizer[Action] {

      case Insert(entity: Entity, assignments) =>
        val (table, columns, values) = insertInfo(insertEntityTokenizer, entity, assignments)
        stmt"INSERT $table${actionAlias.map(alias => stmt" AS ${alias.token}").getOrElse(stmt"")} (${columns.mkStmt(
          ","
            ...
            action match {
              case Insert(entity: Entity, assignments) =>
                ...
                stmt"INSERT $table${actionAlias.map(alias => stmt" AS ${alias.token}").getOrElse(stmt"")} (${columns
                  .mkStmt(",")}) OUTPUT ${returnListTokenizer.token(
                  ExpandReturning(r, Some("INSERTED"))(this, strategy).map(_._1)
                )} VALUES (${values.map(scopedTokenizer(_)).mkStmt(", ")})"
              case Update(_, _)                        =>
  ...
  private def insertInfo(insertEntityTokenizer: Tokenizer[Entity], entity: Entity, assignments: List[Assignment])(
