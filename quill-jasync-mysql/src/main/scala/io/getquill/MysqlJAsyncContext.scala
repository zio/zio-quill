...
class MysqlJAsyncContext[+N <: NamingStrategy](naming: N, pool: ConnectionPool[MySQLConnection])
    extends JAsyncContext[MySQLDialect, N, MySQLConnection](MySQLDialect, naming, pool)
  ...
  override protected def extractActionResult[O](returningAction: ReturnAction, returningExtractor: Extractor[O])(
    ...
    result match {
      case r: MySQLQueryResult =>
        List(
          returningExtractor(
            new ArrayRowData(0, Map.empty[String, Integer].asJava, Array(JavaLong.valueOf(r.getLastInsertId))),
            ()
          )
        )
      case _ =>
