...
class MysqlJAsyncContext[N <: NamingStrategy](
    naming: N,
    pool: ConnectionPool[MySQLConnection]
) extends JAsyncContext[MySQLDialect, N, MySQLConnection](
      MySQLDialect,
      naming,
      pool
    )
  ...
  def this(naming: N, config: MysqlJAsyncContextConfig) =
    this(naming, config.pool)
  def this(naming: N, config: Config) =
    this(naming, MysqlJAsyncContextConfig(config))
    ...
    result match {
      case r: MySQLQueryResult =>
        returningExtractor(
          new ArrayRowData(
            0,
            Map.empty[String, Integer].asJava,
            Array(JavaLong.valueOf(r.getLastInsertId))
          ),
          ()
        )
      case _ =>
