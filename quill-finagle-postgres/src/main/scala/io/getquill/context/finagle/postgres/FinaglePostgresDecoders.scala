...
trait FinaglePostgresDecoders {
  ...
  implicit def optionDecoder[T](implicit d: Decoder[T]): Decoder[Option[T]] =
    FinaglePostgresDecoder[Option[T]](
      new ValueDecoder[Option[T]] {
        ...
        def decodeBinary(recv: String, bytes: ByteBuf, charset: Charset): Try[Option[T]] =
  ...
  implicit val dateDecoder: Decoder[Date] =
    decoderMapped[LocalDateTime, Date](d => Date.from(d.atZone(ZoneId.systemDefault()).toInstant))
