...
trait FinaglePostgresEncoders {
  ...
  case class FinaglePostgresEncoder[T](encoder: ValueEncoder[T])
  ...
  def encoder[T](implicit e: ValueEncoder[T]): Encoder[T] =
    FinaglePostgresEncoder(e)
  ...
  implicit val byteArrayEncoder: Encoder[Array[Byte]] =
    encoder[Array[Byte]](bytea)
  implicit val dateEncoder: Encoder[Date] =
    encoder[LocalDateTime, Date]((v: Date) =>
      LocalDateTime.ofInstant(v.toInstant, ZoneId.systemDefault())
    )
