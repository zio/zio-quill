...
class FinaglePostgresContext[N <: NamingStrategy](val naming: N, client: PostgresClient)
  ...
  def executeAction[T](sql: String,
                       prepare: Prepare = identityPrepare,
                       extractor: Extractor[T] = identityExtractor
  ): Future[Long] = {
    val (params, prepared) = prepare(Nil)
  ...
  def executeBatchAction[B](groups: List[BatchGroup]): Future[List[Long]] = Future
    .collect {
  ...
  def executeActionReturning[T](sql: String,
                                prepare: Prepare = identityPrepare,
    ...
    val (params, prepared) = prepare(Nil)
    logger.logQuery(sql, params)
    withClient(_.prepareAndQuery(expandAction(sql, returningAction), prepared: _*)(extractor)).map(v =>
      handleSingleResult(v.toList)
    )
  }
