...
case class FinaglePostgresContextConfig(config: Config) {
  ...
  def hostConnectionLimit: Int =
    Try(config.getInt("hostConnectionLimit")).getOrElse(1)
  def numRetries: Int = Try(config.getInt("numRetries")).getOrElse(4)
  def customTypes: Boolean =
    Try(config.getBoolean("customTypes")).getOrElse(false)
  def customReceiveFunctions
      : PartialFunction[String, ValueDecoder[T] forSome { type T }] =
    ValueDecoder.decoders orElse { case "noop" => ValueDecoder.unknown }
  def binaryResults: Boolean =
    Try(config.getBoolean("binaryResults")).getOrElse(false)
  def binaryParams: Boolean =
    Try(config.getBoolean("binaryParams")).getOrElse(false)

  private def clientSimple: Client = Postgres
    .Client()
    .withCredentials(username, password)
    .database(database)
    .withCustomReceiveFunctions(customReceiveFunctions)
    .withSessionPool
    .maxSize(hostConnectionLimit)
  ...
  private def clientCustomTypes: Client =
    if (!customTypes) clientSimple.withDefaultTypes()
    else clientSimple.configured(CustomTypes(None))

  private def clientWithSSL: Client =
    if (useSsl) clientCustomTypes.withTransport.tls else clientCustomTypes
