...
abstract class NdbcContext[
    I <: SqlIdiom,
    N <: NamingStrategy,
    P <: PreparedStatement,
    R <: Row
](
    override val idiom: I,
    override val naming: N,
    val dataSource: DataSource[P, R]
  ...
  override implicit protected val resultEffect
      : NdbcContextBase.ContextEffect[Future, Unit] = NdbcContext.ContextEffect

  override type TranslateResult[T] = Future[T]

  def wrap[T](t: => T): Future[T] = resultEffect.wrap(t)
  def push[A, B](result: Future[A])(f: A => B): Future[B] =
    resultEffect.push(result)(f)
  ...
  )(info: ExecutionInfo, dc: Runner): Future[O] =
  ...
  override def withDataSource[T](f: DataSource[P, R] => Future[T]): Future[T] =
    f(dataSource)
...
object NdbcContext {
  object ContextEffect extends NdbcContextBase.ContextEffect[Future, Unit] {
    ...
    override def seq[T](list: List[Future[T]]): Future[List[T]] =
      Future.sequence(list)

    override def runBlocking[T](eff: Future[T], timeout: Duration): T =
      Await.result(eff, timeout)
  }
}
