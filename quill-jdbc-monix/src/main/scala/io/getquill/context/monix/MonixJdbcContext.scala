...
abstract class MonixJdbcContext[Dialect <: SqlIdiom, Naming <: NamingStrategy](
  ...
  override private[getquill] val logger = ContextLogger(
    classOf[MonixJdbcContext[_, _]]
  )
  ...
  )(info: ExecutionInfo, dc: Runner): Task[List[T]] =
  ...
  )(info: ExecutionInfo, dc: Runner): Task[O] =
  ...
  )(info: ExecutionInfo, dc: Runner): Task[List[Long]] =
  ...
  override def prepareQuery(sql: String, prepare: Prepare)(
      info: ExecutionInfo,
      dc: Runner
  ): Connection => Task[PreparedStatement] =
    super.prepareQuery(sql, prepare)(info, dc)
  override def prepareAction(sql: String, prepare: Prepare)(
      info: ExecutionInfo,
      dc: Runner
  ): Connection => Task[PreparedStatement] =
    super.prepareAction(sql, prepare)(info, dc)
  override def prepareBatchAction(groups: List[BatchGroup])(
      info: ExecutionInfo,
      dc: Runner
  ): Connection => Task[List[PreparedStatement]] =
  ...
  override protected def withConnection[T](f: Connection => Task[T]): Task[T] =
    for {
      ...
      result <- maybeConnection match {
        ...
        case None =>
          wrapper.schedule {
            wrap(dataSource.getConnection).bracket(f)(conn =>
              wrapper.wrapClose(conn.close())
            )
          }
      }
    } yield result

  protected def withConnectionObservable[T](
      f: Connection => Observable[T]
  ): Observable[T] =
    for {
      ...
      result <- maybeConnection match {
        ...
        case None =>
          wrapper.scheduleObservable {
            Observable
              .eval(dataSource.getConnection)
              .bracket(conn => withAutocommitBracket(conn, f))(conn =>
                wrapper.wrapClose(conn.close())
              )
          }
      }
  ...
  /** Need to store, set and restore the client's autocommit mode since some
    * vendors (e.g. postgres) don't like autocommit=true during streaming
    * sessions. Using brackets to do that.
    */
  private[getquill] def withAutocommitBracket[T](
      conn: Connection,
      f: Connection => Observable[T]
  ): Observable[T] = {
    Observable
      .eval(autocommitOff(conn))
  ...
  private[getquill] def withCloseBracket[T](
      conn: Connection,
      f: Connection => Task[T]
  ): Task[T] = {
    Task(conn)
  ...
  private[getquill] def autocommitOff(
      conn: Connection
  ): (Connection, Boolean) = {
      ...
      .suspend(
        ...
        currentConnection().map(_ => f).getOrElse {
          wrapper
            ...
            .bracket { conn =>
              TaskLocal
                .wrap(Task.pure(currentConnection))
                .flatMap { tl =>
              ...
              wrapper.wrapClose {
                conn.setAutoCommit(
                  true
                ) // Do we need this if we're closing anyway?
                conn.close()
              }
            }
        }
  ...
  /** In order to allow a ResultSet to be consumed by an Observable, a ResultSet
    * iterator must be created. Since Quill provides a extractor for an
    * individual ResultSet row, a single row can easily be cached in memory.
    * This allows for a straightforward implementation of a hasNext method.
    */
  class ResultSetIterator[T](
      rs: ResultSet,
      conn: Connection,
      extractor: Extractor[T]
  ) extends collection.BufferedIterator[T] {
  ...
  override private[getquill] def prepareParams(
    ...
    withConnectionWrapped { conn =>
      prepare(conn.prepareStatement(statement), conn)._1.reverse
        .map(prepareParam)
    }
  }
}
...
object MonixJdbcContext {
  object EffectWrapper {
    ...
    def using(scheduler: Scheduler) = new EffectWrapper {
      ...
      override def boundary[T](t: Task[T]): Task[T] =
        t.executeOn(scheduler, true)
      override def scheduleObservable[T](o: Observable[T]): Observable[T] =
        o.executeOn(scheduler, true)
    }
  }
  ...
  trait EffectWrapper {
    ...
    /** Use this method whenever a ResultSet is being wrapped. This has a
      * distinct method because the client may prefer to fail silently on a
      * ResultSet close as opposed to failing the surrounding task.
      */
