...
abstract class MonixJdbcContext[+Dialect <: SqlIdiom, +Naming <: NamingStrategy](
  ...
  override def executeActionReturningMany[O](
    ...
    extractor: Extractor[O],
    returningBehavior: ReturnAction
  )(info: ExecutionInfo, dc: Runner): Task[List[O]] =
  ...
  )(info: ExecutionInfo, dc: Runner): Connection => Task[List[PreparedStatement]] =
  ...
  private[getquill] def withAutocommitBracket[T](conn: Connection, f: Connection => Observable[T]): Observable[T] =
    Observable.eval(autocommitOff(conn)).bracket { case (conn, _) => f(conn) }(autoCommitBackOn)

  private[getquill] def withAutocommitBracket[T](conn: Connection, f: Connection => Task[T]): Task[T] =
    Task(autocommitOff(conn)).bracket { case (conn, _) => f(conn) }(autoCommitBackOn)
      ...
      .suspend(
        ...
        currentConnection().map(_ => f).getOrElse {
          wrapper.wrap {
            ...
            TaskLocal.wrap(Task.pure(currentConnection)).flatMap { tl =>
   ...
   * individual ResultSet row, a single row can easily be cached in memory. This
   * allows for a straightforward implementation of a hasNext method.
   */
  class ResultSetIterator[T](rs: ResultSet, conn: Connection, extractor: Extractor[T])
