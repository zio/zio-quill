...
class CassandraLagomAsyncContext[N <: NamingStrategy](
  ...
  ): CassandraLagomSession => Future[BoundStatement] = (session: Session) => {
    val prepareResult =
      session.cs.prepare(cql).map(bs => prepare(bs.bind(), session))
  ...
  def prepareBatchAction[T](
      groups: List[BatchGroup]
  )(info: ExecutionInfo, dc: DatasourceContext)(implicit
      executionContext: ExecutionContext
  ): CassandraLagomSession => Future[List[BoundStatement]] =
    (session: Session) => {
  ...
  def executeQuery[T](
    ...
    val statement =
      prepareAsyncAndGetStatement(cql, prepare, wrappedSession, logger)
    statement
      .flatMap(st => session.selectAll(st))
      .map(_.map(row => extractor(row, wrappedSession)))
  }
  ...
  )(implicit executionContext: ExecutionContext): Result[RunActionResult] = {
    val statement =
      prepareAsyncAndGetStatement(cql, prepare, wrappedSession, logger)
