...
class CassandraLagomAsyncContext[N <: NamingStrategy](
  ...
  def prepareAction[T](cql: String, prepare: Prepare = identityPrepare)(implicit
      executionContext: ExecutionContext
  ): CassandraSession => Future[BoundStatement] = (session: Session) => {
  ...
  def prepareBatchAction[T](groups: List[BatchGroup])(implicit
      executionContext: ExecutionContext
  ): CassandraSession => Future[List[BoundStatement]] = (session: Session) => {
    val batches = groups.flatMap { case BatchGroup(cql, prepares) =>
      prepares.map(cql -> _)
    }
    Future.traverse(batches) { case (cql, prepare) =>
  ...
  def executeQuerySingle[T](cql: String,
  ...
  def executeAction[T](cql: String, prepare: Prepare = identityPrepare)(implicit
      executionContext: ExecutionContext
  ...
  )(implicit executionContext: ExecutionContext): Result[RunBatchActionResult] =
    Future
      .sequence {
        groups.flatMap { case BatchGroup(cql, prepares) =>
