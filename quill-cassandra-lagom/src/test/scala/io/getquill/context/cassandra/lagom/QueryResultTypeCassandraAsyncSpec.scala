...
class QueryResultTypeCassandraAsyncSpec extends QueryResultTypeCassandraSpec {
  ...
  "bind" - {
    "action" - {
      "noArgs" in {
        val bs = result(
          context.prepare(insert(OrderTestEntity(1, 2)))(context.wrappedSession)
        )
      ...
      "withArgs" in {
        val bs = result(
          context.prepare(insert(lift(OrderTestEntity(1, 2))))(
            context.wrappedSession
          )
        )
    ...
    "query" - {
      ...
      "withArgs" in {
        val batches = result(
          context.prepare(
            liftQuery(List(OrderTestEntity(1, 2))).foreach(e => insert(e))
          )(context.wrappedSession)
        )
        batches.foreach { bs =>
          bs.preparedStatement().getVariables.size() mustEqual 2
