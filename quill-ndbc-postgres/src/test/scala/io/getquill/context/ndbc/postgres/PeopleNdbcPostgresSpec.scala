...
class PeopleNdbcPostgresSpec extends PeopleSpec {
  ...
  "Example 5 - compose" in {
    get(
      context.run(`Ex 5 compose`(lift(`Ex 5 param 1`), lift(`Ex 5 param 2`)))
    ) mustEqual `Ex 5 expected result`
  }

  "Example 6 - predicate 0" in {
    get(
      context.run(satisfies(eval(`Ex 6 predicate`)))
    ) mustEqual `Ex 6 expected result`
  }

  "Example 7 - predicate 1" in {
    get(
      context.run(satisfies(eval(`Ex 7 predicate`)))
    ) mustEqual `Ex 7 expected result`
  }

  "Example 8 - contains empty" in {
    get(
      context.run(`Ex 8 and 9 contains`(liftQuery(`Ex 8 param`)))
    ) mustEqual `Ex 8 expected result`
  }

  "Example 9 - contains non empty" in {
    get(
      context.run(`Ex 8 and 9 contains`(liftQuery(`Ex 9 param`)))
