...
class DepartmentsNdbcPostgresSpec extends DepartmentsSpec {
    ...
    get {
      context.transaction {
        for {
          ...
          _ <- context.run(
            liftQuery(departmentEntries).foreach(e => departmentInsert(e))
          )
          _ <- context.run(
            liftQuery(employeeEntries).foreach(e => employeeInsert(e))
          )
  ...
  "Example 8 - nested naive" in {
    get(
      context.run(`Example 8 expertise naive`(lift(`Example 8 param`)))
    ) mustEqual `Example 8 expected result`
  }

  "Example 9 - nested db" in {
    get(
      context.run(`Example 9 expertise`(lift(`Example 9 param`)))
