...
/** Use to provide `run(myQuery)` calls with a context implicitly saving the
  * need to provide things multiple times. For example in JDBC:
  * {{{
...
object ImplicitSyntax {
  ...
  implicit
  final class ImplicitSyntaxOps[R, E, A](private val self: ZIO[R, E, A])
      extends AnyVal {
