...
class Introspection[T](t: T)(implicit tt: ru.TypeTag[T], ct: ClassTag[T]) {
  ...
  val fieldsAndValues: List[(String, Any)] =
    ...
    }.map(sym => (sym.name.toString, instanceMirror.reflectField(sym.asTerm).get)).toList.reverse

  val map = fieldsAndValues.toMap

  def getIndex(i: Int): Any       = fieldsAndValues(i - 1)._2 // Subtract 1 because DB result sets are 1-indexed
  def getField(name: String): Any = map(name)
}
