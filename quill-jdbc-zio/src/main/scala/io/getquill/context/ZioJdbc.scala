...
object ZioJdbc {
    ...
    private[getquill] val layer = {
      val managed =
        for {
          ...
          r    <- managedBestEffort(
                    ZIO.effect(from.get.getConnection).refineToOrDie[SQLException]: ZIO[Any, SQLException, Connection]
                  )
        } yield Has(r)
  ...
  object QConnection {
    ...
    @deprecated(
      "Use qzio.onDataSource.provide(Has(ds)). If you need just SQLException add .refineToOrDie[SQLException]",
      "3.8.0"
    )
    def provideConnectionFrom[T](qzio: ZIO[Has[Connection], Throwable, T])(
  ...
  def managedBestEffort[R, E, A <: AutoCloseable](effect: ZIO[R, E, A]) =
    ZManaged.make(effect)(resource =>
      ZIO
        .effect(resource.close())
        .tapError(e => ZIO.effect(logger.underlying.error(s"close() of resource failed", e)).ignore)
        .ignore
    )
