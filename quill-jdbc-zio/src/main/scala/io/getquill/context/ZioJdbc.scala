...
object ZioJdbc {
  ...
  implicit class DataSourceLayerExt(
      layer: ZLayer[Any, Throwable, Has[DataSource with Closeable]]
  ) {
    def widen: ZLayer[Any, Throwable, Has[DataSource]] =
      layer.map(ds => Has(ds.get[DataSource with Closeable]: DataSource))
  }
  ...
  object DataSourceLayer {
    ...
    private[getquill] val layer = {
      val managed =
        for {
          blockingExecutor <- ZIO
            .succeed(zio.blocking.Blocking.Service.live.blockingExecutor)
            .toManaged_
          from <- ZManaged.environment[Has[DataSource]]
          r <- ZioJdbc
            .managedBestEffort(ZIO.effect(from.get.getConnection))
            .refineToOrDie[SQLException]
            .lock(blockingExecutor)
        } yield Has(r)
    ...
    def fromDataSource(
        ds: => DataSource
    ): ZLayer[Any, Throwable, Has[DataSource]] =
    ...
    def fromJdbcConfig(
      ...
      ZLayer.fromManagedMany(
        for {
          ...
          ds <- managedBestEffort(
            Task(conf.dataSource: DataSource with Closeable)
          )
        } yield Has(ds)
      )

    def fromConfigClosable(
        config: => Config
    ): ZLayer[Any, Throwable, Has[DataSource with Closeable]] =
      fromJdbcConfigClosable(JdbcContextConfig(config))

    def fromPrefixClosable(
        prefix: String
    ): ZLayer[Any, Throwable, Has[DataSource with Closeable]] =
  ...
  implicit class QuillZioDataSourceExt[T](
      qzio: ZIO[Has[DataSource], Throwable, T]
  ) {
    import io.getquill.context.qzio.ImplicitSyntax._
    def implicitDS(implicit
        implicitEnv: Implicit[Has[DataSource]]
    ): ZIO[Any, SQLException, T] =
      (for {
        q <- qzio.provide(implicitEnv.env)
      } yield q).refineToOrDie[SQLException]
  }

  implicit class QuillZioSomeDataSourceExt[T, R <: Has[_]](
      qzio: ZIO[Has[DataSource] with R, Throwable, T]
  )(implicit tag: Tag[R]) {
    import io.getquill.context.qzio.ImplicitSyntax._
    def implicitSomeDS(implicit
        implicitEnv: Implicit[Has[DataSource]]
    ): ZIO[R, SQLException, T] =
      (for {
        r <- ZIO.environment[R]
        q <- qzio
  ...
  implicit class QuillZioExtPlain[T](qzio: ZIO[Has[Connection], Throwable, T]) {
    ...
    @deprecated(
      "Preferably use a ZioJdbcContext instead of a ZioJdbcUnderlyingContext when starting with a DataSource " +
        ...
        "use .provideLayer(DataSourceLayer.live)",
      "3.10.0"
    )
    def implicitDS(implicit
        implicitEnv: Implicit[Has[DataSource]]
  ...
  implicit class QuillZioExt[T, R <: Has[_]](
      qzio: ZIO[Has[Connection] with R, Throwable, T]
  )(implicit tag: Tag[R]) {

    /** Change `Has[Connection]` of a QIO to `Has[DataSource with Closeable]` by
      * providing a `DataSourceLayer.live` instance which will grab a connection
  ...
  /** This is the same as `ZManaged.fromAutoCloseable` but if the `.close()`
    ...
    * state (e.g. failing to close a transaction before closing the connection
    * or failing to release a stale connection) which will eventually cause
    * other operations (i.e. future reads/writes) to fail that have not occurred
    * yet.
    */
  private[getquill] def managedBestEffort[R, E, A <: AutoCloseable](
      effect: ZIO[R, E, A]
  ) =
    ZManaged.make(effect)(resource =>
      blockingEffect(resource.close())
        .tapError(e =>
          ZIO
            .effect(logger.underlying.error(s"close() of resource failed", e))
            .ignore
        )
        .ignore
    )
  ...
  private[getquill] val streamBlocker: ZStream[Any, Nothing, Any] =
    ZStream
      .managed(zio.blocking.blockingExecutor.toManaged_.flatMap { executor =>
