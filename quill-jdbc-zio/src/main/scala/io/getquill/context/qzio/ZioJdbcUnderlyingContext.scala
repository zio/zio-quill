...
abstract class ZioJdbcUnderlyingContext[
    Dialect <: SqlIdiom,
  ...
  override private[getquill] val logger = ContextLogger(
    classOf[ZioJdbcUnderlyingContext[_, _]]
  )
  ...
  )(info: ExecutionInfo, dc: Runner): QCIO[List[Long]] =
  ...
  private def sqlEffect[T](t: => T): QCIO[T] =
    ZIO.effect(t).refineToOrDie[SQLException]

  private[getquill] def withoutAutoCommit[R <: Has[
    Connection
  ], A, E <: Throwable: ClassTag](f: ZIO[R, E, A]): ZIO[R, E, A] = {
    for {
      blockingConn <- ZIO.environment[Has[Connection]]
      conn = blockingConn.get[Connection]
      autoCommitPrev = conn.getAutoCommit
      r <- sqlEffect(conn)
        .bracket(conn => UIO(conn.setAutoCommit(autoCommitPrev))) { conn =>
  ...
  private[getquill] def streamWithoutAutoCommit[A](
      f: ZStream[Has[Connection], Throwable, A]
  ): ZStream[Has[Connection], Throwable, A] = {
    for {
      blockingConn <- ZStream.environment[Has[Connection]]
      conn = blockingConn.get[Connection]
      autoCommitPrev = conn.getAutoCommit
      r <- ZStream
        .bracket(Task(conn.setAutoCommit(false)))(_ =>
  ...
  def transaction[R <: Has[Connection], A](
      f: ZIO[R, Throwable, A]
  ): ZIO[R, Throwable, A] = {
    ZIO
      .environment[R]
            ...
            f.onExit {
              ...
              case Failure(cause) =>
                UIO(env.get[Connection].rollback()).foldCauseM(
                  ...
                  rollbackFailCause =>
                    ZIO.halt(cause.flatMap(Cause.die) ++ rollbackFailCause),
                  _ =>
                    ZIO.halt(
                      cause.flatMap(Cause.die)
                    ) // or ZIO.halt(cause).orDie
                )
            }.provide(env)
          )
        )
  ...
  def streamQuery[T](
    ...
    val managedEnv
        : ZStream[Connection, Throwable, (Connection, PrepareRow, ResultSet)] =
    ...
    val typedStream =
      outStream.provideSome((bc: Has[Connection]) => bc.get[Connection])
    // Run the chunked fetch on the blocking pool
    streamBlocker *> streamWithoutAutoCommit(typedStream)
  ...
  override def push[A, B](result: ZIO[Has[Connection], SQLException, A])(
      f: A => B
  ): ZIO[Has[Connection], SQLException, B] = result.map(f)
  override def seq[A](
      f: List[ZIO[Has[Connection], SQLException, A]]
  ): ZIO[Has[Connection], SQLException, List[A]] = ZIO.collectAll(f)
}
