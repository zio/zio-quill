...
abstract class ZioJdbcContext[Dialect <: SqlIdiom, Naming <: NamingStrategy]
  ...
  override def prepareQuery[T](sql: String,
                               prepare: Prepare,
                               extractor: Extractor[T] = identityExtractor
  ): QIO[PreparedStatement]                                                                 =
  ...
  private[getquill] def withoutAutoCommit[A, E <: Throwable: ClassTag](
      f: ZIO[Has[Connection], E, A]
  ): ZIO[Has[Connection], E, A] =
    for {
  ...
  private[getquill] def streamWithoutAutoCommit[A](
    ...
    for {
      ...
      autoCommitPrev = conn.getAutoCommit
      r             <-
        ZStream.bracket(Task(conn.setAutoCommit(false)))(_ => UIO(conn.setAutoCommit(autoCommitPrev))).flatMap(_ => f)
    } yield r
    ...
    probingDataSource match {
      case Some(dataSource) =>
        Try {
          ...
          try c.createStatement().execute(sql)
          finally c.close()
        }
      case None             => Try[Unit](())
    }
  ...
  def streamQuery[T](fetchSize: Option[Int],
                     sql: String,
                     prepare: Prepare = identityPrepare,
                     extractor: Extractor[T] = identityExtractor
  ): QStream[T] = {
  ...
  override protected val effect = new ContextEffect[Result] {
    ...
    override def push[A, B](result: ZIO[Has[Connection], SQLException, A])(
        f: A => B
    ): ZIO[Has[Connection], SQLException, B]                                                                         =
      throw new IllegalArgumentException("Runner not used for zio context.")
