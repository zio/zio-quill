...
/** Quill context that executes JDBC queries inside of ZIO. Unlike most other
  ...
  * java.sql.Connection as a resource dependency which can be provided later
  ...
  * `ZIO[Has[DataSource], SQLException, T]`. The extension method
  * `.onDataSource` in `io.getquill.context.ZioJdbc.QuillZioExt` will perform
  * this conversion (for even more brevity use `onDS` which is an alias for this
  * method). {{ import ZioJdbc._ val zioDs =
  * DataSourceLayer.fromPrefix("testPostgresDB")
...
abstract class ZioJdbcContext[Dialect <: SqlIdiom, Naming <: NamingStrategy]
  ...
  override def executeQuerySingle[T](
    ...
    onConnection(
      underlying.executeQuerySingle[T](sql, prepare, extractor)(info, dc)
    )
  ...
  override def translateQuery[T](
    ...
    onConnection(
      underlying.translateQuery[T](statement, prepare, extractor, prettyPrint)(
        executionInfo,
        dc
      )
    )
  ...
  override def translateBatchQuery(
    ...
    onConnection(
      underlying.translateBatchQuery(
        groups.asInstanceOf[List[ZioJdbcContext.this.underlying.BatchGroup]],
        prettyPrint
      )(executionInfo, dc)
    )
  ...
  def executeActionReturning[O](
    ...
    onConnection(
      underlying.executeActionReturning[O](
        sql,
        prepare,
        extractor,
        returningBehavior
  ...
  def executeBatchAction(
      groups: List[BatchGroup]
  )(info: ExecutionInfo, dc: Runner): QIO[List[Long]] =
    onConnection(
      underlying.executeBatchAction(
        groups.asInstanceOf[List[ZioJdbcContext.this.underlying.BatchGroup]]
      )(info, dc)
    )
  ...
  def executeBatchActionReturning[T](
    ...
    onConnection(
      underlying.executeBatchActionReturning[T](
        groups.asInstanceOf[List[
          ZioJdbcContext.this.underlying.BatchGroupReturning
        ]],
        extractor
      )(info, dc)
    )

  def prepareQuery(sql: String, prepare: Prepare)(
      info: ExecutionInfo,
      dc: Runner
  ): QCIO[PreparedStatement] =
    underlying.prepareQuery(sql, prepare)(info, dc)

  def prepareAction(sql: String, prepare: Prepare)(
      info: ExecutionInfo,
      dc: Runner
  ): QCIO[PreparedStatement] =
  ...
  /** Execute instructions in a transaction. For example, to add a Person row to
    ...
    */
  def transaction[R <: Has[DataSource], A](
    ...
    withBlocking(currentConnection.get.flatMap {
      ...
      case None =>
        val connection = for {
          ...
          _ <- ZManaged.make(blockingEffect(connection.setAutoCommit(false))) {
            _ =>
  ...
  private def onConnection[T](
      qlio: ZIO[Has[Connection], SQLException, T]
  ...
  private def onConnectionStream[T](
      qstream: ZStream[Has[Connection], SQLException, T]
  ): ZStream[Has[DataSource], SQLException, T] =
