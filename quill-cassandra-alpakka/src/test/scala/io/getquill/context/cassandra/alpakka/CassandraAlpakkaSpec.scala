...
trait CassandraAlpakkaSpec extends Spec {
  ...
  val alpakkaSession: CassandraAlpakkaSession = CassandraSessionRegistry
    .get(actorSystem)
  ...
  lazy val testDB
      : CassandraAlpakkaContext[Literal.type] with CassandraTestEntities =
    new CassandraAlpakkaContext(
      Literal,
      alpakkaSession,
