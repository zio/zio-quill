...
import akka.stream.alpakka.cassandra.scaladsl.{
  CassandraSession => CassandraAlpakkaSession
}
import akka.stream.scaladsl.Source
import akka.{Done, NotUsed}
import com.datastax.oss.driver.api.core.CqlSession
import com.datastax.oss.driver.api.core.cql.{BoundStatement, PreparedStatement}
import io.getquill.context.cassandra.{
...
class CassandraAlpakkaContext[N <: NamingStrategy](
  ...
  override lazy val session: CqlSession =
    Await.result(alpakkaSession.underlying(), 30.seconds)

  lazy val asyncCache = new PrepareStatementCache[Future[PreparedStatement]](
  ...
  override def performIO[T](io: IO[T, _], transactional: Boolean = false)(
    ...
    if (transactional)
      logger.underlying.warn(
        "Cassandra doesn't support transactions, ignoring `io.transactional`"
      )
    super.performIO(io)
  }
  ...
  def streamQuery[T](
    ...
    alpakkaSession
      .select(prepareAsyncAndGetStatement(cql, prepare, this, logger))
  ...
  def executeQuery[T](
    ...
    statement
      .flatMap(st => alpakkaSession.selectAll(st))
      .map(rows => rows.map(row => extractor(row, this)).toList)
  }
