...
object LinkedHashMapOps {
  implicit class LinkedHashMapExt[K, V](m1: mutable.LinkedHashMap[K, V]) {
    def zipWith[R](m2: mutable.LinkedHashMap[K, V])(
  ...
  def outerZipWith[K, V, R](
    ...
    mutable
      .LinkedHashSet((m1.keySet.toList ++ m2.keySet.toList): _*)
      .map(k => (k, m1.get(k), m2.get(k)))
      .collect(f)
  }
}

/** This represents a simplified Sql-Type. Since it applies to all dialects, it
  * is called Quill-Application-Type hence Quat. Quats represent the types of
  * the Quill AST allowing us to know what fields exist on an Ident and more
  ...
  * String etc... More specific Quat.Value types are planned int the future.
  * <li> Quat.Generic - A Quat representing a type whose value is not known yet.
  * <li> Quat.Null - A Quat representing a null value. </ul>
  *
  * Quats are generally treated as through they are immutable although there are
  * dynamic/mutable variables in some places (e.g. Product uses LinkedHashSet
  * and var) due to various other issues (e.g. performance, serialization). It
  * is assumed that all operations Quats have referential transparency.
  */
sealed trait Quat {
  def isAbstract =
    this match {
  ...
  def probit =
    this match {
      ...
      case other =>
        QuatException(
          s"Was expecting SQL-level type must be a Product but found `${other}`"
        )
    }
  ...
  def leastUpperType(other: Quat): Option[Quat] = {
    (this, other) match {
      ...
      case (Quat.BooleanExpression, Quat.BooleanExpression) =>
        Some(Quat.BooleanExpression)
      case (Quat.BooleanValue, Quat.BooleanValue) => Some(Quat.BooleanValue)
      case (Quat.BooleanValue, Quat.BooleanExpression) =>
        Some(Quat.BooleanValue)
      ...
      case (me: Quat.Product, other: Quat.Product) =>
        me.leastUpperTypeProduct(other)
      case (_, _) => None
    }
  }
  ...
  def shortString: String = this match {
    case p @ Quat.Product(fields) =>
      s"CC${if (p.tpe == Quat.Product.Type.Abstract) "A" else ""}(${fields
        .map { case (k, v) =>
          k + (v match {
  ...
  /** What was the value of a given property before it was renamed (i.e. looks
    * up the value of the Renames hash)
    */
  def beforeRenamed(path: String): Option[String] = (this, path) match {
    ...
    case (other, fieldName) =>
      QuatException(
        s"The post-rename field '${fieldName}' does not exist in an SQL-level type ${other}"
      )
  }
  ...
  def lookup(path: String, failNonExist: Boolean): Quat = (this, path) match {
      ...
      fields.get(fieldName).getOrElse {
        if (failNonExist)
          throw new IllegalArgumentException(
            s"The field '${fieldName}' does not exist in an SQL-level type ${cc}"
          )
        else {
    ...
    case (other, fieldName) =>
      if (failNonExist)
        throw new IllegalArgumentException(
          s"The field '${fieldName}' does not exist in an SQL-level type ${other}"
        )
      else {
        io.getquill.util.Messages.trace(
  ...
  def lookup(list: List[String], failNonExist: Boolean): Quat =
    list match {
      case head :: tail =>
        this.lookup(head, failNonExist).lookup(tail, failNonExist)
      case Nil => this
    }
}
...
object Quat {
  ...
  def improveInfixQuat(ast: Ast) = {
    ast match {
      ...
      case i @ Infix(parts, List(param), pure, transparent, _)
          if (transparent) =>
  ...
  def fromSerializedJVM(serial: String): Quat =
    KryoQuatSerializer.deserialize(serial)
  def fromSerializedJS(serial: String): Quat =
    BooQuatSerializer.deserialize(serial)
  ...
  class Product(
    ...
    def withRenamesFrom(other: Quat): Quat = {
      other match {
        case otherProduct: Quat.Product =>
          val newFields =
            fields
              ...
              .map {
                ...
                case Right((key, from: Product, to: Product)) =>
                  (key, to.withRenamesFrom(from))
          ...
          val newTpe =
            if (
              this.tpe == Product.Type.Abstract || otherProduct.tpe == Product.Type.Abstract
            )
          ...
          Quat
            .Product(newFields)
            .withRenames(otherProduct.renames)
            .withType(newTpe)
    ...
    def leastUpperTypeProduct(other: Quat.Product): Option[Quat.Product] = {
      ...
      val newTpe =
        if (
          this.tpe == Product.Type.Abstract || other.tpe == Product.Type.Abstract
        )
          Product.Type.Abstract
        else
          Product.Type.Concrete
    ...
    override def withRenames(
        renames: mutable.LinkedHashMap[String, String]
    ): Quat.Product =
    ...
    override def withRenames(renames: List[(String, String)]): Quat.Product =
      Product.WithRenames(
        ...
        fields,
        (mutable
          .LinkedHashMap[String, String]() ++ renames): mutable.LinkedHashMap[
          String,
          String
        ]
      )
  ...
  def LeafTuple(numElems: Int) =
    Quat.Tuple((1 to numElems).map(_ => Quat.Value))

  object Product {
    case class Id(fields: mutable.LinkedHashMap[String, Quat])

    def fromSerializedJVM(serial: String): Quat.Product =
      KryoQuatSerializer.deserialize(serial).probit
    def fromSerializedJS(serial: String): Quat.Product =
      BooQuatSerializer.deserialize(serial).probit

    def empty = new Quat.Product(
      mutable.LinkedHashMap(),
      mutable.LinkedHashMap(),
      Type.Concrete
    )

    def apply(fields: (String, Quat)*): Quat.Product = new Quat.Product(
      mutable.LinkedHashMap[String, Quat]() ++ fields.iterator,
      mutable.LinkedHashMap(),
      Type.Concrete
    )
    def apply(tpe: Type, fields: (String, Quat)*): Quat.Product =
      new Quat.Product(
        mutable.LinkedHashMap[String, Quat]() ++ fields.iterator,
        mutable.LinkedHashMap(),
        tpe
    ...
    def apply(fields: Iterable[(String, Quat)]): Quat.Product =
      new Quat.Product(
        mutable.LinkedHashMap[String, Quat]() ++ fields.iterator,
        mutable.LinkedHashMap(),
        Type.Concrete
      )
    def apply(tpe: Type, fields: Iterable[(String, Quat)]): Quat.Product =
      new Quat.Product(
        mutable.LinkedHashMap[String, Quat]() ++ fields.iterator,
        mutable.LinkedHashMap(),
        tpe
    ...
    def apply(fields: Iterator[(String, Quat)]): Quat.Product =
      new Quat.Product(
        mutable.LinkedHashMap[String, Quat]() ++ fields,
        mutable.LinkedHashMap(),
        Type.Concrete
      )
    def apply(tpe: Type, fields: Iterator[(String, Quat)]): Quat.Product =
      new Quat.Product(
        mutable.LinkedHashMap[String, Quat]() ++ fields,
        mutable.LinkedHashMap(),
        tpe
    ...
    def apply(fields: mutable.LinkedHashMap[String, Quat]): Quat.Product =
      new Quat.Product(fields, mutable.LinkedHashMap(), Type.Concrete)
    def apply(
        tpe: Type,
        fields: mutable.LinkedHashMap[String, Quat]
    ): Quat.Product = new Quat.Product(fields, mutable.LinkedHashMap(), tpe)

    def unapply(p: Quat.Product): Option[mutable.LinkedHashMap[String, Quat]] =
      Some(p.fields)
    ...
    /** Since Product-Quats can be a representation of abstract product types
      ...
      * TypeMemberJoinSpec, there are certain situations where the contents of
      * an `Ident(a)` are not entirely known and the ident needs to be expanded
      * as either `struct(a.*)` or just `a.*` depending on the situation.
      */
    sealed trait Type
    object Type {
    ...
    object WithRenames {
      ...
      def iterated(
        ...
        WithRenames.apply(
          tpe,
          (mutable
            .LinkedHashMap[String, Quat]() ++ list): mutable.LinkedHashMap[
            String,
            Quat
          ],
          (mutable
    ...
    object WithRenamesCompact {
      def apply(tpe: Quat.Product.Type)(
          fields: String*
        ...
        Product.WithRenames.iterated(
          tpe,
          fields.zip(values).iterator,
          renamesFrom.zip(renamesTo).iterator
        )
      }
  ...
  object Tuple {
    ...
    def apply(fields: Iterable[Quat]): Quat.Product =
      Quat.Product(fields.zipWithIndex.map { case (f, i) =>
        (s"_${i + 1}", f)
      }.iterator)
  }
  case object Null extends Quat {
    override def withRenames(renames: mutable.LinkedHashMap[String, String]) =
      this
  }
  ...
  protected trait NoRenames {
    ...
    override def withRenames(
      ...
      if (renames.isEmpty)
        ...
        QuatException(
          s"Renames $renames cannot be applied to a value SQL-level type"
        )
    }
  }
}
