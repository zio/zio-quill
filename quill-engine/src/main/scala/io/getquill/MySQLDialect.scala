...
trait MySQLDialect
    ...
    def tokenizer(implicit astTokenizer: Tokenizer[Ast]) =
      Tokenizer[OnConflict] {
        ...
        case OnConflict(i: io.getquill.ast.Action, NoTarget, Ignore) =>
          actionTokenizer(insertIgnoreTokenizer)(actionAstTokenizer, strategy)
            .token(i)

        case _ =>
          fail(
            "This upsert construct is not supported in MySQL. Please refer documentation for details."
          )
      }
      ...
      Tokenizer.withFallback[Ast](MySQLDialect.this.astTokenizer(_, strategy)) {
        case Property.Opinionated(Excluded(_), name, renameable, _) =>
          renameable.fixedOr(name.token)(
            stmt"VALUES(${strategy.column(name).token})"
          )
  ...
  override implicit def orderByCriteriaTokenizer(implicit
    ...
    case OrderByCriteria(prop, DescNullsFirst) =>
      stmt"ISNULL(${prop.token}) DESC, ${prop.token} DESC"
    case OrderByCriteria(prop, AscNullsLast) =>
      stmt"ISNULL(${prop.token}) ASC, ${prop.token} ASC"
  ...
  override protected def limitOffsetToken(
    ...
    Tokenizer[(Option[Ast], Option[Ast])] {
      case (None, Some(offset)) =>
        stmt"$query LIMIT 18446744073709551610 OFFSET ${offset.token}"
