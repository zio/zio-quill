...
trait MySQLDialect
  ...
  override def defaultAutoGeneratedToken(field: Token): Statement = stmt"($field) VALUES (DEFAULT)"
    ...
    def tokenizer(implicit astTokenizer: Tokenizer[Ast]) =
      Tokenizer[OnConflict] {
        ...
        case OnConflict(i: io.getquill.ast.Action, NoTarget, Ignore) =>
          actionTokenizer(insertIgnoreTokenizer)(actionAstTokenizer, strategy, idiomContext).token(i)
