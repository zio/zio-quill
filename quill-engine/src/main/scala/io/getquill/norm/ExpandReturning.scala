...
/** Take the `.returning` part in a query that contains it and return the array
  * of columns representing of the returning seccovtion with any other
  * operations etc... that they might contain.
  */
  ...
  def applyMap(
    ...
    idiom.idiomReturningCapability match {
      ...
      case ReturningMultipleFieldSupported =>
        ...
        ReturnColumns(initialExpand.map { case (ast, statement) =>
          f(ast, statement)
        })
  ...
  def apply(
    ...
    val dePropertized = renameAlias match {
      case Some(newName) =>
        BetaReduction(
          properties,
          alias -> ExternalIdent.Opinionated(newName, alias.quat, Fixed)
        )
      case None =>
        BetaReduction(
          properties,
          alias -> ExternalIdent(alias.name, alias.quat)
        )
    }
    ...
    val outputs = deTuplified.map(v =>
      idiom.translate(v, dePropertized.quat, ExecutionType.Unknown)
    )
