...
/** Due to the introduction of null checks in `map`, `flatMap`, and `exists`, in
  ...
  * query[Holder].map(h => h.value.map(_ + "foo").getOrElse("bar")) // Yields
  * the following result: SELECT CASE WHEN CASE WHEN h.value IS NOT NULL THEN
  * h.value || 'foo' ELSE null END IS NOT NULL THEN CASE WHEN h.value IS NOT
  * NULL THEN h.value || 'foo' ELSE null END ELSE 'bar' END FROM Holder h
  * </pre></code> This of course is highly redundant and can be reduced to
  * simply: <code><pre> SELECT CASE WHEN h.value IS NOT NULL AND (h.value ||
  * 'foo') IS NOT NULL THEN h.value || 'foo' ELSE 'bar' END FROM Holder h
  * </pre></code> This reduction is done by the "Center Rule." There are some
  * other simplification rules as well. Note how we are force to null-check both
  * `h.value` as well as `(h.value || 'foo')` because a user may use
  * `Option[T].flatMap` and explicitly transform a particular value to `null`.
  */
class SimplifyNullChecks(equalityBehavior: EqualityBehavior)
  ...
  override def apply(ast: Ast): Ast =
    ast match {
      ...
      case (left +&&+ OptionIsEmpty(Optional(Constant(_, _)))) +||+ other =>
        apply(other)
      case (OptionIsEmpty(Optional(Constant(_, _))) +&&+ right) +||+ other =>
        apply(other)
      case other +||+ (left +&&+ OptionIsEmpty(Optional(Constant(_, _)))) =>
        apply(other)
      case other +||+ (OptionIsEmpty(Optional(Constant(_, _))) +&&+ right) =>
        apply(other)

      case (left +&&+ OptionIsDefined(Optional(Constant(_, _)))) => apply(left)
      case (OptionIsDefined(Optional(Constant(_, _))) +&&+ right) =>
        apply(right)
  ...
  /** Simple extractor that looks inside of an optional values to see if the
    * thing inside can be pulled out. If not, it just returns whatever element
    * it can find.
    */
  object Optional {
