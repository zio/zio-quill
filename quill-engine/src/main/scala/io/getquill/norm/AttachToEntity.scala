...
/** Find the innermost clause that contains an entity and attach some other kind
  ...
  * }}}
  *
  * Note how the inner ident `{tmp}` needs to be unique and not conflict with
  * any ident higher in the AST that is used inside the Cond clause, otherwise,
  * the various kinds of ASTs will be irreversibly corrupted. Here is an
  * example:
  *
  * Take:
  * {{{
  ...
  * }}}
  * This is of course completely incorrect because the ident {a1} should
  * actually be {a} referring to the ident of the outer FlatMap.
  */
object AttachToEntity {
  ...
  def apply(f: (Ast, Ident) => Query, alias: Option[Ident] = None)(
      q: Ast
  ): Ast = applyWithId(f, alias, 0)(q)

  private def applyWithId(
      f: (Ast, Ident) => Query,
      alias: Option[Ident],
      nextId: Long
  )(q: Ast): Ast =
    q match {
      ...
      case Map(a: Query, b, c) =>
        Map(applyWithId(f, Some(b), nextId + 1)(a), b, c)
      case FlatMap(a: Query, b, c) =>
        FlatMap(applyWithId(f, Some(b), nextId + 1)(a), b, c)
      case ConcatMap(a: Query, b, c) =>
        ConcatMap(applyWithId(f, Some(b), nextId + 1)(a), b, c)
      case Filter(a: Query, b, c) =>
        Filter(applyWithId(f, Some(b), nextId + 1)(a), b, c)
      case SortBy(a: Query, b, c, d) =>
        SortBy(applyWithId(f, Some(b), nextId + 1)(a), b, c, d)
      case Take(a: Query, b) => Take(applyWithId(f, alias, nextId + 1)(a), b)
      case Drop(a: Query, b) => Drop(applyWithId(f, alias, nextId + 1)(a), b)
      case Aggregation(op, a: Query) =>
        Aggregation(op, applyWithId(f, alias, nextId + 1)(a))
      case Distinct(a: Query) => Distinct(applyWithId(f, alias, nextId + 1)(a))

      case IsEntity(q) =>
        f(q, alias.getOrElse(Ident(s"[tmp_attachtoentity${nextId}]", q.quat)))
