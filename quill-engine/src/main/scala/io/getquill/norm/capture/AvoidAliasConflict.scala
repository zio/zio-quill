...
/** This phase of normalization ensures that symbol names in every layer of the
  ...
  * variables (e.g. as created by the `AttachToEntity` phase) into permanant
  * ones of the form x[0-9]+. Since `AvoidAliasConflict` typically runs not on
  * the entire Ast but the sub-parts of it used by normalizations, making
  * temporary aliases permanent cannot be done in these sub-parts because the
  * 'state' of this transformation is not fully know
  ...
  */
private[getquill] case class AvoidAliasConflict(
    state: Set[IdentName],
    detemp: Boolean
  ...
  private def recurseAndApply[T <: Query](elem: T)(
      ext: T => (Ast, Ident, Ast)
  )(f: (Ast, Ident, Ast) => T): (T, StatefulTransformer[Set[IdentName]]) =
        ...
        trace"Uncapturing Join $qq" andReturn {
          ...
          val (orr, orrt) =
            trace"Uncapturing Join: Recurse with state: ${brt.state} + $freshA + $freshB" andReturn
              AvoidAliasConflict(
                brt.state + freshA.idName + freshB.idName,
                detemp
              )(or)
  ...
  private def apply(x: Ident, p: Ast)(
      f: (Ident, Ast) => Query
  ): (Query, StatefulTransformer[Set[IdentName]]) =
  ...
  /** If the ident is temporary (e.g. given by [tmp_{UUID}] give it an actual
    * variable and the make sure it does not conflict with any other variables
    * of outer clauses in the AST (freshIdent does that part).
    */
...
private[getquill] object AvoidAliasConflict {
  ...
  /** Make sure query parameters do not collide with paramters of a AST
    * function. Do this by walkning through the function's subtree and
    * transforming and queries encountered.
    */
  def sanitizeVariables(
      f: Function,
      dangerousVariables: Set[IdentName]
  ): Function = {
