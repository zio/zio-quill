...
/** A problem occurred in the original way infixes were done in that it was
  ...
  * e.g. `RAND()`, and most importantly `RANK()`. For this reason, the
  * operations that are done in `ApplyMap` on standard AST `Map` clauses cannot
  * be done therefore additional safety checks were introduced there in order to
  * assure this does not happen. In addition to this however, it is necessary to
  * add this normalization step which inserts `Nested` AST elements in every map
...
object NestImpureMappedInfix extends StatelessTransformer {
  ...
  override def apply(ast: Ast): Ast =
    ast match {
      ...
      case m @ Map(q, x, i @ Infix(_, _, false, _, _)) =>
