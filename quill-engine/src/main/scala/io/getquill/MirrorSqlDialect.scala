...
object MirrorSqlDialect extends MirrorSqlDialect {
  ...
  trait StrategizeElements
    ...
    override def tokenizeIdentName(
        strategy: NamingStrategy,
        name: String
    ): String = strategy.default(name)
    override def tokenizeTableAlias(
        strategy: NamingStrategy,
        table: String
    ): String = strategy.default(table)
    override def tokenizeColumnAlias(
        strategy: NamingStrategy,
        column: String
    ): String = strategy.default(column)
    override def tokenizeFixedColumn(
        strategy: NamingStrategy,
        column: String
    ): String = strategy.default(column)
...
object MirrorSqlDialectWithOutputClause
    extends MirrorSqlDialectWithOutputClause {
  override def prepareForProbing(string: String) = string
}
...
object MirrorSqlDialectWithBooleanLiterals
    extends MirrorSqlDialectWithBooleanLiterals {
  override def prepareForProbing(string: String) = string
}
