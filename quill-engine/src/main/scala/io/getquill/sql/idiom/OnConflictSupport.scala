...
trait OnConflictSupport {
  ...
  implicit def conflictTokenizer(implicit astTokenizer: Tokenizer[Ast], strategy: NamingStrategy, transpileConfig: TranspileConfig): Tokenizer[OnConflict] = {
    ...
    val customAstTokenizer =
      Tokenizer.withFallback[Ast](self.astTokenizer(_, strategy, transpileConfig)) {
        case _: OnConflict.Excluded => stmt"EXCLUDED"
        case OnConflict.Existing(a) => stmt"${a.token}"
        case a: Action              => self.actionTokenizer(customEntityTokenizer)(actionAstTokenizer, strategy, transpileConfig).token(a)
      }
