...
trait SqlIdiom extends Idiom {
  ...
  implicit def assignmentTokenizer(implicit
    astTokenizer: Tokenizer[Ast],
    strategy: NamingStrategy
  ): Tokenizer[Assignment] = Tokenizer[Assignment] { case a @ Assignment(_, prop, value) =>
  ...
  implicit def assignmentDualTokenizer(implicit
    astTokenizer: Tokenizer[Ast],
    strategy: NamingStrategy
  ): Tokenizer[AssignmentDual] = Tokenizer[AssignmentDual] { case AssignmentDual(_, _, prop, value) =>
    ...
    val columns =
      assignments.map(assignment =>
        assignment.property match {
          case Property.Opinionated(_, key, renameable, _) => tokenizeColumn(strategy, key, renameable).token
