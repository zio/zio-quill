...
/** Simple utility that checks if if an AST entity refers to a entity. It
  ...
  * sure NOT to apply a naming-schema on them) we need to take care to
  * understand that we may not know the FromContext that a property comes from
...
case class InContext(from: List[FromContext]) {
  ...
  private def collectTableAliases(
      contexts: List[FromContext]
  ): Map[String, InContextType] = {
    contexts
      .map {
        ...
        case JoinContext(_, a, b, _) =>
          collectTableAliases(List(a)) ++ collectTableAliases(List(b))
...
case class SelectPropertyProtractor(from: List[FromContext]) {
  ...
  def apply(
      ast: Ast,
      alternateQuat: Option[Quat]
  ): List[(Ast, List[String])] = {
    ast match {
      case id @ Core() =>
        ...
        effectiveQuat match {
          case p: Quat.Product =>
            ProtractQuat(isEntity)(p, id).map { case (prop, path) =>
              (freezeNonEntityProps(prop, isEntity), path)
            }
          case _ =>
      ...
      case prop @ PropertyMatroshka(id @ Core(), _, _) =>
        ...
        effectiveQuat match {
          case p: Quat.Product =>
            ProtractQuat(isEntity)(p, prop).map { case (prop, path) =>
              (freezeNonEntityProps(prop, isEntity), path)
            }
          case _ =>
