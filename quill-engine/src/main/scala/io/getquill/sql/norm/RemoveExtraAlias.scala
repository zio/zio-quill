...
/** Remove aliases at the top level of the AST since they are not needed (quill
  ...
  */
case class RemoveExtraAlias(strategy: NamingStrategy)
    extends StatelessQueryTransformer {
  ...
  private def removeUnneededAlias(value: SelectValue): SelectValue =
    value match {
      ...
      case sv @ SelectValue(p: Property, Some(alias), _)
          if !strategyMayChangeColumnName(p) && p.name == alias =>
        sv.copy(alias = None)
      case _ =>
        value
    }

  override protected def expandNested(
      q: FlattenSqlQuery,
      level: QueryLevel
  ): FlattenSqlQuery = {
