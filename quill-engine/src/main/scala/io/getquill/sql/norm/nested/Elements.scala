...
object Elements {
  ...
  /** In order to be able to reconstruct the original ordering of elements
    * inside of a select clause, we need to keep track of their order, not only
    * within the top-level select but also it's order within any possible
