...
object Messages {
  ...
  def quatKryoPoolSize = cache(
    "quill.quat.kryoPool",
    variable("quill.quat.kryoPool", "quill_quat_kryoPool", "10").toInt
  )
  def maxQuatFields = cache(
    "quill.quat.tooManyFields",
    variable(
      "quill.quat.tooManyFields",
      "quill_quat_tooManyFields",
      "500"
    ).toInt
  )
  def attachTopLevelQuats = cache(
    ...
    variable(
      "quill.quat.attachTopLevel",
      "quill_quat_attachTopLevel",
      "true"
    ).toBoolean
  )
  def strictQuatChecking = cache(
    "quill.quat.strict",
    variable("quill.quat.strict", "quill_quat_strict", "false").toBoolean
  )
  def prettyPrint = cache(
    "quill.macro.log.pretty",
    variable("quill.macro.log.pretty", "quill_macro_log", "false").toBoolean
  )
  def alwaysAlias = cache(
    "quill.query.alwaysAlias",
    variable(
      "quill.query.alwaysAlias",
      "quill_query_alwaysAlias",
      "false"
    ).toBoolean
  )
  ...
  def traceQuats = cache(
    "quill.trace.quat",
    QuatTrace(
      variable("quill.trace.quat", "quill_trace_quat", QuatTrace.None.value)
    )
  )
  ...
  def querySubexpand = cache(
    "quill.query.subexpand",
    variable("quill.query.subexpand", "query_query_subexpand", "true").toBoolean
  )
  def quillLogFile = cache(
    "quill.log.file",
    LogToFile(variable("quill.log.file", "quill_log_file", "false"))
  )
  ...
  def enableTrace(
      ...
      traceTypes: List[TraceType] =
        List(TraceType.SqlNormalizations, TraceType.Standard)
  ): Unit = {
    ...
    System.setProperty(
      "quill.trace.types",
      traceTypes.map(_.value).mkString(",")
    )
    resetCache()
    ()
  }
