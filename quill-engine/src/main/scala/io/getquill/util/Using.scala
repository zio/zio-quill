...
/** Copied from scala 2.13 source code.
  ...
  * the one of a higher severity is re-thrown, and the one of a lower severity
  * is added to it as a suppressed exception. If the two exceptions are of the
  * same severity, the one thrown first is re-thrown, and the one thrown second
  * is added to it as a suppressed exception. If an exception is a
...
object Using {
  ...
  final class Manager private {
    ...
    /** Registers the specified resource with this manager, so that the resource
      * is released when the manager is closed, and then returns the
      * (unmodified) resource.
      */
  ...
  object Manager {
    ...
    /** Performs an operation using a `Manager`, then closes the `Manager`,
      ...
      * @return
      *   a Try containing an exception if one or more were thrown, or the
      *   result of the operation if no exceptions were thrown
      */
  ...
  /** Performs an operation using three resources, and then releases the
    * resources in reverse order, even if the operation throws an exception.
    * This method behaves similarly to Java's try-with-resources.
    *
    * $suppressionBehavior
    *
    * @param resource1
  ...
  /** Performs an operation using four resources, and then releases the
    * resources in reverse order, even if the operation throws an exception.
    * This method behaves similarly to Java's try-with-resources.
    *
    * $suppressionBehavior
  ...
  /** A typeclass describing how to release a particular type of resource.
    ...
    * considered a resource, because it is important to close the stream after
    * it is finished being used.
    *
    * An instance of `Releasable` is needed in order to automatically manage a
    * resource with Using `Using`. An implicit instance is provided for all
    * types extending java.lang.AutoCloseable.
    *
    * @tparam R
    *   the type of the resource
    */
