...
trait SqliteDialect
  ...
  private[this] val omittedNullsOrdering =
    stmt"omitted (not supported by sqlite)"
  private[this] val omittedNullsFirst =
    stmt"/* NULLS FIRST $omittedNullsOrdering */"
  private[this] val omittedNullsLast =
    stmt"/* NULLS LAST $omittedNullsOrdering */"
  ...
  override implicit def valueTokenizer(implicit
    ...
    case Constant(v: Boolean, _) if v =>
      stmt"1" // TODO Quat.BooleanValue change
