  ...
  override protected def limitOffsetToken(
    ...
    Tokenizer[(Option[Ast], Option[Ast])] {
      ...
      case (Some(limit), Some(offset)) =>
        stmt"$query OFFSET ${offset.token} ROWS FETCH FIRST ${limit.token} ROWS ONLY"
  ...
  override implicit def sqlQueryTokenizer(implicit
    ...
    Tokenizer[SqlQuery] {
      case flatten: FlattenSqlQuery
          if flatten.orderBy.isEmpty && flatten.offset.nonEmpty =>
  ...
  override implicit def operationTokenizer(implicit
    ...
    Tokenizer[Operation] {
      case BinaryOperation(a, StringOperator.`+`, b) =>
        stmt"${scopedTokenizer(a)} + ${scopedTokenizer(b)}"
