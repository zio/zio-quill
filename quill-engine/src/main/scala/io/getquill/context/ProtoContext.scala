...
/** A common context used between Quill and ProtoQuill. This is more like a
  ...
  * various higher-level context extend it as a guard-rail against API drift
  * i.e. so that the Scala2-Quill and ProtoQuill internal-context APIs remain
  * largely the same.
  */
trait ProtoContext[Dialect <: io.getquill.idiom.Idiom, Naming <: NamingStrategy]
    extends RowContext {
  type PrepareRow
  type ResultRow
  ...
  )(executionInfo: ExecutionInfo, rn: Runner): Result[RunQuerySingleResult[T]]
  def executeAction(sql: String, prepare: Prepare)(
      executionInfo: ExecutionInfo,
      rn: Runner
...
/** Metadata related to query execution. Note that AST should be lazy so as not
  * to be evaluated at runtime (which would happen with a by-value property
  * since `{ ExecutionInfo(stuff, ast) } is spliced into a query-execution
  * site). Additionally, there are performance overheads even splicing the
  * finalized version of the AST into call sites of the `run` functions. For
  * this reason, this functionality is being used only in ProtoQuill and only
  * when a trait extends the trait AstSplicing. In the future it might
  * potentially be controlled by a compiler argument.
  */
class ExecutionInfo(
    val executionType: ExecutionType,
    queryAst: => Ast,
    queryTopLevelQuat: => Quat
) {
...
trait ProtoStreamContext[
    Dialect <: io.getquill.idiom.Idiom,
  ...
  def streamQuery[T](
      ...
      prepare: Prepare,
      extractor: Extractor[T]
  )(info: ExecutionInfo, rn: Runner): StreamResult[T]
}
