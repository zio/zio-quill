...
/** Returning a single field from Insert/Update/etc... clauses is supported.
  * This is the most common databases e.g. MySQL, Sqlite, and H2 (although as of
...
/** An actual `RETURNING` clause is supported in the SQL dialect of the
  * specified database e.g. Postgres. this typically means that columns returned
  * from Insert/Update/etc... clauses can have other database operations done on
  * them such as arithmetic `RETURNING id + 1`, UDFs `RETURNING udf(id)` or
  * others. In JDBC, the following is done: `connection.prepareStatement(sql,
  * Statement.RETURN_GENERATED_KEYS))`.
  */
sealed trait ReturningClauseSupported extends ReturningCapability

/** An actual `OUTPUT` clause is supported in the SQL dialect of the specified
  * database e.g. MSSQL Server. this typically means that columns returned from
  * Insert/Update/etc... clauses can have arithmetic operations done on them
  * such as `OUTPUT INSERTED.id + 1`.
  */
