...
trait WithStandardCodegen {
  ...
  def standardCodegen(
      ...
      tableFilter: RawSchema[JdbcTableMeta, JdbcColumnMeta] => Boolean = _ =>
        true,
      entityNamingStrategy: NameParser = LiteralNames,
      entityNamespacer: Namespacer[JdbcTableMeta] = ts =>
        ts.tableSchem.getOrElse(defaultNamespace),
      entityMemberNamer: JdbcQuerySchemaNaming = ts =>
        ts.tableName.snakeToLowerCamel
  ) =
    new JdbcGeneratorBase(() => {
      ...
      override def filter(tc: RawSchema[TableMeta, ColumnMeta]): Boolean =
        super.filter(tc) && tableFilter(tc)
