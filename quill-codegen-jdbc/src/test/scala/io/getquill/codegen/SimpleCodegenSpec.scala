...
class SimpleCodegenSpec extends AnyFreeSpec with Matchers {
  ...
  def assertStandardObject(
    ...
    ob match {
            ...
            case (schema, methodTree) => {
              methodTree match {
                case q"$mods def $tname(...$paramss): $tpt = $expr" => {
                  assert(
                    tname.toString.unquote == schema.defName,
                    s"Def method ${tname} should be ${schema.defName}"
                  )
                  assert(
                    paramss.length == 0,
                    s"Def method ${tname} should not have any params for $tname"
                  )
                  ...
                  quotedExpr match {
                    case q"querySchema[..$qs_tpts](...$qs_args)" => {
                      ...
                      args.toList.head match {
                        case q"$value" =>
                          assert(
                            value.toString.unquote == schema.tableName,
                            err
                          )
                      }
                        ...
                        .foreach({
                          case ((field, tableColumn), schemaFieldTree) =>
                            schemaFieldTree match {
                              case q"(..$params) => $expr" => {
                                ...
                                expr match {
                                  case q"$inputArg.$mapRhs -> $mapLhs" => {
                                    ...
                                    assert(
                                      mapLhs.toString.unquote == tableColumn,
                                      err
                                    )
                                  }
                                  case _ => fail(err)
                              ...
                              case _ =>
                                fail(
                                  s"Invalid datatype in schema definition ${locName} in schema ${schema.defName}"
                                )
                            }
                        })
                    }
                ...
                case _ =>
                  fail(
                    s"Pattern match for schema member ${schema.defName} failed in $otname"
                  )
              }
            }
          })
      ...
      case _ =>
        fail(s"Object code does not match objectCode pattern in  ${objectCode}")
    }
  }
    ...
    cc match {
      case q"case class $tpname(...$paramss) extends { ..$earlydefns } with ..$parents" => {
        ...
        if (constructorList.length != 1)
          fail(s"Class $tpname has more then one constructor list")
        val paramList: Seq[_] = constructorList.toList(0).toList
        if (paramList.length != fields.length)
          fail(
            s"List of fields did not match list of case class params in ${tpname}"
          )
            ...
            fieldTree match {
              case q"$mods val $name: $tpe = $rhs" => {
                assert(
                  ...
                  s"Field ${name.toString} does not match ${fieldName} in $tpname"
                )
                assert(
              ...
              case _ =>
                fail(
                  s"Parsed Tree $fieldTree did not match Field/Type ($fieldName, $fieldType)"
                )
            }
          })
      }
