    ...
    "custom naming strateogy" - {
      ...
      "single table" in {
        ...
        assertStandardObject(
          ...
          "person",
          Seq(
            QuerySchema("person", "PUBLIC.PERSON", personData.querySchemaList)
          )
        )
      }
      ...
      "prefix collision - different columns without datatype perculation" - {
        ...
        "prefix test with snake case" in {
          val gens = standardCodegen(
            ...
            entityNamingStrategy = SnakeCaseCustomTable(
              _.tableName.toLowerCase.replaceFirst("(alpha_)|(bravo_)", "")
            )
          ).makeGenerators.toList.sortBy(_.caseClassesCode)
      ...
      "prefix collision - different columns with datatype perculation" - {
        ...
        "prefix test with snake case - with different columns - and different types" in {
          val gens = standardCodegen(
            ...
            entityNamingStrategy = SnakeCaseCustomTable(
              _.tableName.toLowerCase
                .replaceFirst("(alpha_)|(bravo_)", "")
                .capitalize
            )
          ).makeGenerators.toList.sortBy(_.caseClassesCode)
      ...
      "namespace collision - different columns with datatype perculation" - {
        ...
        "prefix test with snake case - with different columns - and different types" in {
          val gens = standardCodegen(
            ...
            entityNamespacer = _.tableSchem
              .map(_.toLowerCase.replaceAll("(alpha)|(bravo)", "public"))
              .getOrElse(this.defaultNamespace),
            entityMemberNamer = ts =>
              s"${ts.tableSchem.get}_${ts.tableName}".toLowerCase.snakeToLowerCamel
          ).makeGenerators.toList.sortBy(_.caseClassesCode)
