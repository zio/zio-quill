...
import io.getquill.codegen.integration.SchemaNames.{
  simpleLiteral,
  simpleSnake,
  twoSchema
}
...
import io.getquill.codegen.util.{
  ConfigPrefix,
  SchemaConfig,
  SchemaMaker,
  SchemaMakerCoordinates
}
...
sealed trait CodegenTestCases {
  ...
  def schemaMakerCoordinates(dbPrefix: ConfigPrefix) =
    SchemaMakerCoordinates(dbPrefix, naming, schemaConfig)

  def generateWithSchema(
  ...
  protected def `package`(implicit dbPrefix: ConfigPrefix) =
    (pathList :+ s"`${testName}`").mkString(".")
}
...
object CodegenTestCases {
  ...
  case object `1-simple-snake`
      extends TestInfo(SnakeCase, simpleSnake)
      with `1-simple-snake` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val snakeCaseGen = new SimpleJdbcCodegen(dbPrefix.loadConfig, `package`)
        with DatabaseBasedNumericTyping {
        override def defaultNamespace: String = "public"
        override def nameParser = SnakeCaseNames
        override def filter(
            tc: RawSchema[JdbcTableMeta, JdbcColumnMeta]
        ): Boolean = schemaFilter(databaseType, tc) && super.filter(tc)
        override def namespacer: Namespacer[JdbcTableMeta] = ts => {
          if (super.namespacer(ts) startsWith "codegenTest") "public"
          else super.namespacer(ts)
        }
      }
  ...
  case object `2-simple-literal`
      extends TestInfo(SnakeCase, simpleLiteral)
      with `2-simple-literal` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val literalGen = new SimpleJdbcCodegen(dbPrefix.loadConfig, `package`)
        with DatabaseBasedNumericTyping {
        override def defaultNamespace: String = "public"
  ...
  case object `1-comp-sanity`
    ...
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      ...
      implicit val base = basePath
      val gen =
        new ComposeableTraitsJdbcCodegen(dbPrefix.loadConfig, `package`, false)
          with DatabaseBasedNumericTyping {
          override def defaultNamespace: String = "public"
  ...
  case object `2-comp-stereo-single`
      extends TestInfo(SnakeCase, simpleSnake)
      with `2-comp-stereo-single` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val gen =
  ...
  case object `3-comp-stereo-oneschema`
      extends TestInfo(SnakeCase, twoSchema)
      with `3-comp-stereo-oneschema` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val gen = new ComposeableTraitsJdbcCodegen(
        ...
        override def querySchemaNaming: JdbcQuerySchemaNaming = ts => {
          ...
          (if (schema startsWith "codegenTest") "public"
           else schema) + ts.tableName.toLowerCase.capitalize
        }
      }
  ...
  case object `4-comp-stereo-twoschema`
      extends TestInfo(SnakeCase, twoSchema)
      with `4-comp-stereo-twoschema` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val gen =
        new ComposeableTraitsJdbcCodegen(databaseConnections, `package`, false)
          with DatabaseBasedNumericTyping {
          override def defaultNamespace: String = "public"
          override def nameParser: NameParser = CustomNames()
          override def filter(
            ...
            if (schema == "alpha" || schema == "bravo") "common"
              ...
              databaseType match {
                case MySql | Oracle | SqlServer
                    if (schema startsWith "codegenTest") =>
                  "public"
  ...
  case object `5-comp-non-stereo-allschema`
      extends TestInfo(SnakeCase, twoSchema)
      with `5-comp-non-stereo-allschema` {
    override def generate(dbPrefix: ConfigPrefix, basePath: String) = {
      implicit val prefix = dbPrefix
      implicit val base = basePath
      val gen = new ComposeableTraitsJdbcCodegen(
  ...
  private[integration] def databaseConnections(implicit prefix: ConfigPrefix) =
    prefix match {
      case TestOracleDB =>
        Seq(
          JdbcContextConfig(prefix.loadConfig).dataSource,
          JdbcContextConfig(
            prefix.loadConfig.withValue(
              "dataSource.user",
              ConfigValueFactory.fromAnyRef("alpha")
            )
          ).dataSource,
          JdbcContextConfig(
            prefix.loadConfig.withValue(
              "dataSource.user",
              ConfigValueFactory.fromAnyRef("bravo")
            )
          ).dataSource
      ...
      case _ =>
        Seq(() => JdbcContextConfig(prefix.loadConfig).dataSource.getConnection)
    }

  private[integration] def schemaFilter(
