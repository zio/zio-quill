...
sealed trait SchemaConfig {
  ...
  lazy val content: String = {
    val content = Source
      .fromURL(this.getClass.getClassLoader.getResource(fileName))
      .mkString
...
object SchemaConfig {
  ...
  case object `schema_snakecase_twoschema_differentcolumns_differenttypes`
  ...
  case object `schema_snakecase_twotable_differentcolumns_differenttypes`
      extends SchemaConfig
