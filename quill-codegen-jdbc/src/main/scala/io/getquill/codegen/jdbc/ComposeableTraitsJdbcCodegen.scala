...
/** This generator generates a query schema trait which can be composed with a
  * custom context that you create in your client code (called
  * MySchemaExtensions below because it extends and existing quill context with
  * your query schemas). Here is what that looks like:
  *
  ...
  * io.getquill.NamingStrategy] { this:io.getquill.context.Context[Idiom,
  * Naming] =>
  *
  ...
  *
  * <h2>When DAO Objects Collide</h2> Now when you are trying to generate
  * schemas which are not being stereotyped but have equivalent table names for
  * example: <pre> trait AlphaExtensions[Idiom <: io.getquill.idiom.Idiom,
  * Naming <: io.getquill.NamingStrategy] {
  * this:io.getquill.context.Context[Idiom, Naming] =>
  *
...
class ComposeableTraitsJdbcCodegen(
  ...
  override def makeGenerators: Seq[ContextifiedUnitGenerator] =
    new MultiGeneratorFactory(generatorMaker).apply
  override def generatorMaker =
    new SingleGeneratorFactory[ContextifiedUnitGenerator] {
      override def apply(
          emitterSettings: EmitterSettings[JdbcTableMeta, JdbcColumnMeta]
    ...
    PackagingStrategy.ByPackageHeader
      ...
      .copy(fileNamingStrategy =
        new BySomeTableData[ContextifiedUnitGenerator](gen =>
          Paths
            .get(gen.packageName.getOrElse(gen.defaultNamespace), gen.traitName)
        )
      )

  class ContextifiedUnitGenerator(
      emitterSettings: EmitterSettings[JdbcTableMeta, JdbcColumnMeta]
    ...
    def traitName: String =
      packageName.getOrElse(defaultNamespace).capitalize + "Extensions"
