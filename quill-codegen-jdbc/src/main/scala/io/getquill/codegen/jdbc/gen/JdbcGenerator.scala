...
class JdbcGeneratorBase(
  ...
  def this(connectionMaker: JdbcConnectionMaker) =
    this(Seq(connectionMaker), "")
}

trait JdbcGenerator extends Generator {
  this: JdbcCodeGeneratorComponents with JdbcStereotyper =>
  val connectionMakers: Seq[JdbcConnectionMaker]
  val databaseType: DatabaseType =
    DiscoverDatabaseType.apply(connectionMakers.head)
  val columnGetter = (cm: ColumnMeta) => cm.columnName

  override def filter(tc: RawSchema[JdbcTableMeta, JdbcColumnMeta]): Boolean = {
    databaseType match {
      case MySql =>
        !tc.table.tableCat.existsInSetNocase(defaultExcludedSchemas.toList: _*)
      case _ =>
        !tc.table.tableSchem.existsInSetNocase(
          defaultExcludedSchemas.toList: _*
        )
    }
  }
  ...
  override def namespacer: Namespacer[TableMeta] = databaseType match {
    case MySql | SqlServer =>
      tm => tm.tableCat.map(_.snakeToLowerCamel).getOrElse(defaultNamespace)
    case _ =>
      tm =>
        tm.tableSchem
          .orElse(tm.tableCat)
          .map(_.snakeToLowerCamel)
