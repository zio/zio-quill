...
object DatabaseTypes {
  ...
  def all: Seq[DatabaseType] =
    Seq(H2, MySql, SqlServer, Postgres, Sqlite, Oracle)
      ...
      if (res.isDefined) Success(res.get)
        ...
        Failure(
          new IllegalArgumentException(
            s"Database type ${productName} not supported. Possible Values are: ${all
              .map(_.databaseName)}"
          )
        )
    }
