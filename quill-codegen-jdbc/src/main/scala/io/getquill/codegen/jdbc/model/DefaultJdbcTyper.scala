    ...
    strategy match {
      ...
      case ThrowTypingError =>
        throw new TypingError(
          s"Could not resolve jdbc type: ${jdbcType}" + tag
            .map(t => s" class: `${t}`.")
            .getOrElse("")
        )
    }
    ...
    jdbcType match {
      ...
      case NUMERIC =>
        numericPreference match {
          case PreferPrimitivesWhenPossible
              if (jdbcTypeInfo.size <= MaxIntDigits) =>
            Some(classTag[Int])
          case PreferPrimitivesWhenPossible
              if (jdbcTypeInfo.size <= MaxLongDigits) =>
            Some(classTag[Long])
      ...
      case BINARY | VARBINARY | LONGVARBINARY | BLOB =>
        unresolvedType(jdbcType, classTag[java.sql.Blob])
