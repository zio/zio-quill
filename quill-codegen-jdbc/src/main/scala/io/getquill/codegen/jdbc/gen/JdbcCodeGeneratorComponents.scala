...
trait JdbcCodeGeneratorComponents extends CodeGeneratorComponents {
  ...
  override def defaultExcludedSchemas =
    Set("information_schema", "performance_schema", "sys", "mysql")

  /** When the Jdbc Typer tries to figure out which Scala/Java objects to use
    * for which JDBC type (e.g. use String for Varchar(...), Long for bigint
    * etc...), what do we do when we discover a JDBC type which we cannot
    * translate (e.g. blob which is currently not supported by quill). The
    * simplest thing to do is to skip the column.
    */
  ...
  def packagingStrategy: PackagingStrategy =
    PackagingStrategy.ByPackageHeader.TablePerFile(packagePrefix)
}
