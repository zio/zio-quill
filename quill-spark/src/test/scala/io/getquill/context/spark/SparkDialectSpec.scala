...
class SparkDialectSpec extends Spec {
  ...
  "translate" - {
    "query" in {
      ...
      val (norm, stmt, _) =
        SparkDialect.translate(ast, Quat.Unknown, ExecutionType.Unknown)(
          Literal
        )
      norm mustEqual ast
  ...
  "escapes ' " in {
    ...
    val (norm, stmt, _) =
      SparkDialect.translate(ast, Quat.Unknown, ExecutionType.Unknown)(Literal)
    norm mustEqual ast
  ...
  "concatMap with filter" in {
    val ast =
      query[Test].concatMap(t => t.s.split(" ")).filter(s => s == "s").ast
    val (norm, stmt, _) =
      SparkDialect.translate(ast, Quat.Unknown, ExecutionType.Unknown)(Literal)
