...
case class ParentNestedTupleHolderTest(
    tup: NestedTupleHolderTest,
    otherData: String
)
case class SuperParentNestedTupleHolderTest(
    tup: (NestedTupleHolderTest, Test),
    otherData: String
)
...
class ExpandEntityIdsSpec extends Spec {
  ...
  val s2 = liftQuery(
    Seq(
      SingleElementSingleHolder(SingleElement(1)),
      SingleElementSingleHolder(SingleElement(2))
    ).toDS
  )
  val s3 = liftQuery(
    Seq(
  ...
  "single element nesting cases" - {
    ...
    "two-level single element, nested in tuple" in {
      ...
      testContext.run(q).collect.toList mustEqual List(
        (Test(1, 2, "3"), SingleElementSingleHolder(SingleElement(1)))
      )
    }
  ...
  "allows nested returns" - {
    ...
    "entities inside ad-hoc case classes multiple levels different levels" in {
      ...
      testContext.run(q).collect.toList mustEqual entities.map(e =>
        TestHolderHolderOneSide(TestHolder(e, e), e)
      )
    }
    "entities inside ad-hoc case classes multiple levels mixed with tuples" in {
      val q = quote {
        for {
          ...
          TestHolderHolder(TestHolder(a, b), TestHolder(a, b)),
          TestHolder(a, b)
        )
      }
      testContext.run(q).collect.toList mustEqual entities.map(e =>
  ...
  "advanced tuple nesting cases" - {
    ...
    "test nested tuple holder" in {
      ...
      testContext.run(q).collect.toList mustEqual entities.map(e =>
        NestedTupleHolderTest((e.i, e), e.s)
      )
    }
    "test nested tuple holder inside parent" in {
      val q = quote {
        for {
          ...
          c <- qr3 if (a.i + 1) == c.j
          d <- qr4 if (c.i + 1) == d.j
        } yield ParentNestedTupleHolderTest(
          NestedTupleHolderTest((a.i, b), c.s),
          c.s
        )
      }
      testContext.run(q).collect.toList mustEqual entities.map(e =>
    ...
    "test nested tuple holder inside parent inside super tuple" in {
      val q = quote {
        for {
          ...
          c <- qr3 if (a.i + 1) == c.j
          d <- qr4 if (c.i + 1) == d.j
        } yield SuperParentNestedTupleHolderTest(
          (NestedTupleHolderTest((a.i, b), c.s), c),
          c.s
        )
      }
      testContext.run(q).collect.toList mustEqual entities.map(e =>
        SuperParentNestedTupleHolderTest(
          (NestedTupleHolderTest((e.i, e), e.s), e),
          e.s
        )
      )
    }
