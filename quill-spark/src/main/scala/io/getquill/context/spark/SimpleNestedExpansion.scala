...
object TopLevelExpansion {
  ...
  private def apply(value: SelectValue, length: Int): List[SelectValue] = {
    value match {
      ...
      case SelectValue(
            Ident(singleFieldName, q @ Quat.Product(fields)),
            alias,
            concat
          ) if (length == 1 && q.tpe == Quat.Product.Type.Concrete) =>
        fields.map { case (name, quat) =>
          SelectValue(
            Property(Ident(singleFieldName, quat), name),
            Some(name),
            concat
          )
        }.toList
      ...
      case other @ SelectValue(SingleValuePrimitive(), currentAlias, _)
          if (length == 1) =>
...
/** In each of these situations, the AST can be expanded with as a single
  ...
  * SelectValue(If(foo,bar,baz)) would become 'SELECT CASE WHEN foo THEN bar
  * ELSE baz as single ...', or SelectValue(NullValue) would become 'SELECT null
  * as single ...' In each of these cases we can assume the actual selection
  * represents a single value. The opposite of this if the selection is an
  * Ident. In that case, we can never really assume that the selection
  * represents a single value (*) so we generally have to do star expansions of
  * various kinds. This unapplier object is used both here and in the
  * SpartDialect select tokenization.
  *
  ...
  *     TopLevelExpanion.apply and the Ident shuold no longer exist in the
  *     select values.
  *
  ...
  protected override def apply(q: SqlQuery, level: QueryLevel): SqlQuery =
    q match {
      case q: FlattenSqlQuery => //if (isTopLevel) =>  //needs to be for all levels
        expandNested(
          q.copy(select = TopLevelExpansion(q.select, q.select.length))(q.quat),
          level
        )
      case other =>
        super.apply(q, level)
