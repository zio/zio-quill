...
trait QuillSparkContext
  ...
  implicit val ignoreDecoders: QuatMaking.IgnoreDecoders =
    QuatMaking.IgnoreDecoders
  ...
  case class StructElement(column: Column, structField: StructField) {
    def children: Array[StructElement] = structField.dataType match {
      case StructType(fields) =>
        fields.map(f => StructElement(column.getField(f.name), f))
      case _ => Array()
    }
  }

  /** As a result of converting product objects from <code>value.*</code>
    * selects in the <code>SparkDialect</code> tokenizer, when expressing empty
    * options (e.g. as a result of left joins), we will get a array of null
    * values instead of a single null value like the Spark decode expects. See
    * the issue #123 for more details.
    *
    ...
    * UDF that will nullify the entire row if all of the elements of that row
    * are null. </li> </ol>
    *
  ...
  private def percolateNullArrays[T: SparkEncoder](ds: Dataset[T]) = {
    ...
    def percolateNullArraysRecursive(node: StructElement): Column =
      node.structField.dataType match {
        case st: StructType =>
          ...
          val preculatedColumn = struct(
            node.children.map(percolateNullArraysRecursive(_)): _*
          )
          ...
          val mapped =
            (c: Column) => {
              when(
                st.fieldNames
                  .map(c.getField(_))
                  .foldLeft(lit(true))({ case (curr, col) =>
                    curr && col.isNull
                  }),
                lit(null: Column)
              ).otherwise(c)
    ...
    ds.select(
      ds.schema.fields.map(f =>
        percolateNullArraysRecursive(StructElement(col(f.name), f))
      ): _*
    ).as[T]
  }
  ...
  /** As a result of expanding <code>value.*</code> selects in
    ...
    *
    * However, since the outermost return type must be a tuple, the query must
    * be the following: <br/><code>select struct(f.*) _1, struct(b.*) _2 from
    * Foo f join Bar b on f.id = b.fk</code>
    *
    * These <code>_1</code> and <code>_2</code> aliases are added by this
    * object. In the edge case where they cannot be found (typically this
    * happens when single primitive values are selected), the outputs are
    * assumed to be tuple indexes i.e. _1, _2, etc...
    */
  ...
  private[getquill] def prepareString(string: String, prepare: Prepare)(implicit
      spark: SQLContext
  ) = {
    var dsId = 0
