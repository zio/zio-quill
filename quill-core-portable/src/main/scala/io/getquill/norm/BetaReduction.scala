...
case class BetaReduction(map: IMap[Ast, Ast], typeBehavior: TypeBehavior) extends StatelessTransformer {
  ...
  private def checkQuats(body: Ast, replacements: Seq[(Ast, Ast)]) =
    replacements.foreach { case (orig, rep) =>
      ...
      if (rep.quat.leastUpperType(orig.quat).isEmpty)
        throw new IllegalArgumentException(
          s"Cannot beta reduce [$rep <- $orig] within [$body] because ${rep.quat.shortString} [of:${rep}] is not a subtype of ${orig.quat.shortString} [of:${orig}]"
        )
    }
