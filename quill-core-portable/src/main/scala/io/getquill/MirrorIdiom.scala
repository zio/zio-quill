...
trait MirrorIdiomBase extends Idiom {
  ...
  implicit def operatorTokenizer[T <: Operator]: Tokenizer[T] = Tokenizer[T] { case o =>
    stmt"${o.toString.token}"
  }
  ...
  implicit def conflictTokenizer(implicit externalTokenizer: Tokenizer[External]): Tokenizer[OnConflict] = {
    ...
    val updateAssignsTokenizer = Tokenizer[Assignment] { case Assignment(i, p, v) =>
