...
object Messages {
  ...
  def quatKryoPoolSize: Int = cache("quill.quat.kryoPool", variable("quill.quat.kryoPool", "quill_quat_kryoPool", "10").toInt)
  def maxQuatFields: Int = cache("quill.quat.tooManyFields", variable("quill.quat.tooManyFields", "quill_quat_tooManyFields", "500").toInt)
  def strictQuatChecking: Boolean = cache("quill.quat.strict", variable("quill.quat.strict", "quill_quat_strict", "false").toBoolean)
  def prettyPrint: Boolean = cache("quill.macro.log.pretty", variable("quill.macro.log.pretty", "quill_macro_log", "false").toBoolean)
  def alwaysAlias: Boolean = cache("quill.query.alwaysAlias", variable("quill.query.alwaysAlias", "quill_query_alwaysAlias", "false").toBoolean)
  def pruneColumns: Boolean = cache("quill.query.pruneColumns", variable("quill.query.pruneColumns", "quill_query_pruneColumns", "true").toBoolean)
  def smartBooleans: Boolean = cache("quill.query.smartBooleans", variable("quill.query.smartBooleans", "quill_query_smartBooleans", "true").toBoolean)
  def debugEnabled: Boolean = cache("quill.macro.log", variable("quill.macro.log", "quill_macro_log", "true").toBoolean)
  def traceEnabled: Boolean = cache("quill.trace.enabled", variable("quill.trace.enabled", "quill_trace_enabled", "false").toBoolean)
  def traceColors: Boolean = cache("quill.trace.color", variable("quill.trace.color", "quill_trace_color,", "false").toBoolean)
  def traceOpinions: Boolean = cache("quill.trace.opinion", variable("quill.trace.opinion", "quill_trace_opinion", "false").toBoolean)
  def traceAstSimple: Boolean = cache("quill.trace.ast.simple", variable("quill.trace.ast.simple", "quill_trace_ast_simple", "false").toBoolean)
  def traceQuats: QuatTrace = cache("quill.trace.quat", QuatTrace(variable("quill.trace.quat", "quill_trace_quat", QuatTrace.None.value)))
  def cacheDynamicQueries: Boolean = cache("quill.query.cacheDaynamic", variable("quill.query.cacheDaynamic", "query_query_cacheDaynamic", "true").toBoolean)
