...
final class Entity(val name: String, val properties: List[PropertyAlias])(theQuat: => Quat.Product)(val renameable: Renameable) extends Query {
  ...
  def copy(name: String = this.name, properties: List[PropertyAlias] = this.properties, quat: Quat.Product = this.quat): Entity =
...
object Entity {
  final private case class Id(name: String, properties: List[PropertyAlias])
  ...
  def unapply(e: Entity): Some[(String, List[PropertyAlias], Quat.Product)] = Some((e.name, e.properties, e.quat))
  ...
  object Opinionated {
    ...
    def unapply(e: Entity): Some[(String, List[PropertyAlias], Quat.Product, Renameable)] =
...
final case class SortBy(query: Ast, alias: Ident, criterias: Ast, ordering: Ast) extends Query {
  def quat = query.quat
  def bestQuat: Quat = query.bestQuat
}
...
final case class GroupBy(query: Ast, alias: Ident, body: Ast) extends Query {
  def quat: Quat.Product = Quat.Tuple(body.quat, query.quat)
...
final case class Join(
  ...
  def quat: Quat.Product = Quat.Tuple(a.quat, b.quat)
...
final class Infix(val parts: List[String], val params: List[Ast], val pure: Boolean)(theQuat: => Quat) extends Ast {
  ...
  override def hashCode: Int = id.hashCode()
  def copy(parts: List[String] = this.parts, params: List[Ast] = this.params, pure: Boolean = this.pure, quat: Quat = this.quat): Infix =
    Infix(parts, params, pure, quat)
}
object Infix {
  final private case class Id(val parts: List[String], val params: List[Ast], val pure: Boolean)

  def apply(parts: List[String], params: List[Ast], pure: Boolean, quat: => Quat) =
    new Infix(parts, params, pure)(quat)

  def unapply(i: Infix): Some[(List[String], List[Ast], Boolean, Quat)] = Some((i.parts, i.params, i.pure, i.quat))
}
...
object ExternalIdent {
  ...
  def unapply(e: ExternalIdent): Some[(String, Quat)] = Some((e.name, e.quat))

  object Opinionated {
    def apply(name: String, quat: => Quat, rename: Renameable) =
      new ExternalIdent(name)(quat)(rename)

    def unapply(e: ExternalIdent): Some[(String, Quat, Renameable)] = Some((e.name, e.quat, e.renameable))
  }
}
 ...
 */
final case class Property(ast: Ast, name: String) extends Ast {
  ...
  def quat: Quat = ast.quat.lookup(name, Messages.strictQuatChecking)
  def bestQuat: Quat = ast.quat.lookup(name, false)
  def prevName: Option[String] = ast.quat.beforeRenamed(name)
...
object Property {
  ...
  def unapply(p: Property): Some[(Ast, String)] = Some((p.ast, p.name))
  ...
  object Opinionated {
    ...
    def unapply(p: Property): Some[(Ast, String, Renameable, Visibility)] =
      Some((p.ast, p.name, p.renameable, p.visibility))
  }
}

sealed trait OptionOperation extends Ast
final case class OptionFlatten(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }
final case class OptionGetOrElse(ast: Ast, body: Ast) extends OptionOperation { def quat = body.quat; def bestQuat = body.bestQuat }
...
final case class OptionMap(ast: Ast, alias: Ident, body: Ast) extends OptionOperation { def quat = body.quat; def bestQuat = body.bestQuat }
final case class OptionForall(ast: Ast, alias: Ident, body: Ast)
  extends OptionOperation { def quat = body.quat; def bestQuat = body.bestQuat }
final case class OptionExists(ast: Ast, alias: Ident, body: Ast)
  extends OptionOperation { def quat = body.quat; def bestQuat = body.bestQuat }
final case class OptionContains(ast: Ast, body: Ast) extends OptionOperation { def quat = body.quat; def bestQuat = body.bestQuat }
final case class OptionIsEmpty(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }
final case class OptionNonEmpty(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }
final case class OptionIsDefined(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }
final case class OptionTableFlatMap(ast: Ast, alias: Ident, body: Ast)
  extends OptionOperation { def quat = body.quat; def bestQuat = body.bestQuat }
final case class OptionTableMap(ast: Ast, alias: Ident, body: Ast)
  extends OptionOperation { def quat = body.quat; def bestQuat = body.bestQuat }
...
object OptionNone {
  ...
  def unapply(on: OptionNone): Some[Quat] = Some(on.quat)
}

final case class OptionSome(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }
final case class OptionApply(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }
final case class OptionOrNull(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }
final case class OptionGetOrNull(ast: Ast) extends OptionOperation { def quat = ast.quat; def bestQuat = ast.bestQuat }

sealed trait IterableOperation extends Ast
final case class MapContains(ast: Ast, body: Ast) extends IterableOperation { def quat = body.quat; def bestQuat = body.bestQuat }
final case class SetContains(ast: Ast, body: Ast) extends IterableOperation { def quat = body.quat; def bestQuat = body.bestQuat }
final case class ListContains(ast: Ast, body: Ast) extends IterableOperation { def quat = body.quat; def bestQuat = body.bestQuat }

final case class If(condition: Ast, `then`: Ast, `else`: Ast) extends Ast { def quat = `then`.quat; def bestQuat = `then`.bestQuat } // then and else clauses should have identical quats

final case class Assignment(alias: Ident, property: Ast, value: Ast) extends Ast { def quat = Quat.Value; def bestQuat = quat }
final case class AssignmentDual(alias1: Ident, alias2: Ident, property: Ast, value: Ast) extends Ast { def quat = Quat.Value; def bestQuat = quat }

//************************************************************

sealed trait Operation extends Ast

final case class UnaryOperation(operator: UnaryOperator, ast: Ast) extends Operation { def quat = Quat.BooleanExpression; def bestQuat = quat }
...
final case class FunctionApply(function: Ast, values: List[Ast]) extends Operation { def quat = function.quat; def bestQuat = function.bestQuat }
...
final object Constant {
  final private case class Id(v: Any)

  def apply(v: Any, quat: => Quat): Constant = new Constant(v)(quat)
  def unapply(const: Constant): Some[(Any, Quat)] = Some((const.v, const.quat))
...
final case class CaseClass(values: List[(String, Ast)]) extends Value {
...
final case class Update(query: Ast, assignments: List[Assignment]) extends Action { def quat = query.quat; def bestQuat = query.bestQuat }
final case class Insert(query: Ast, assignments: List[Assignment]) extends Action { def quat = query.quat; def bestQuat = query.bestQuat }
final case class Delete(query: Ast) extends Action { def quat = query.quat; def bestQuat = query.bestQuat }
...
final case class Returning(action: Ast, alias: Ident, property: Ast)
  extends ReturningAction { def quat = property.quat; def bestQuat = property.bestQuat }
final case class ReturningGenerated(action: Ast, alias: Ident, property: Ast)
  extends ReturningAction { def quat = property.quat; def bestQuat = property.bestQuat }

final case class Foreach(query: Ast, alias: Ident, body: Ast) extends Action { def quat = body.quat; def bestQuat = body.bestQuat }
...
object Dynamic {
  ...
  def unapply(d: Dynamic): Some[(Any, Quat)] = Some((d.tree, d.quat))
...
final class ScalarValueLift(val name: String, val value: Any, val encoder: Any)(theQuat: => Quat)
  ...
  def copy(name: String = this.name, value: Any = this.value, encoder: Any = this.encoder, quat: => Quat = this.quat): ScalarValueLift =
    ScalarValueLift(name, value, encoder, quat)
}
object ScalarValueLift {
  final private case class Id(name: String, value: Any, encoder: Any)
  def apply(name: String, value: Any, encoder: Any, quat: => Quat): ScalarValueLift = new ScalarValueLift(name, value, encoder)(quat)
  def unapply(svl: ScalarValueLift): Some[(String, Any, Any, Quat)] = Some((svl.name, svl.value, svl.encoder, svl.quat))
}
...
final class ScalarQueryLift(val name: String, val value: Any, val encoder: Any)(theQuat: => Quat)
  ...
  def copy(name: String = this.name, value: Any = this.value, encoder: Any = this.encoder, quat: => Quat = this.quat): ScalarQueryLift =
...
object ScalarQueryLift {
  ...
  def unapply(l: ScalarQueryLift): Some[(String, Any, Any, Quat)] = Some((l.name, l.value, l.encoder, l.quat))
}
...
final class CaseClassValueLift(val name: String, val value: Any)(theQuat: => Quat) extends CaseClassLift {
  ...
  def copy(name: String = this.name, value: Any = this.value, quat: => Quat = this.quat): CaseClassValueLift =
    CaseClassValueLift(name, value, quat)
}
object CaseClassValueLift {
  final private case class Id(name: String, value: Any)
  def apply(name: String, value: Any, quat: => Quat): CaseClassValueLift = new CaseClassValueLift(name, value)(quat)
  def unapply(l: CaseClassValueLift): Some[(String, Any, Quat)] = Some((l.name, l.value, l.quat))
}
...
object CaseClassQueryLift {
  ...
  def unapply(l: CaseClassQueryLift): Some[(String, Any, Quat)] = Some((l.name, l.value, l.quat))
}
...
final case class ScalarTag(uid: String) extends Tag {
  def quat = Quat.Value
  def bestQuat = quat
