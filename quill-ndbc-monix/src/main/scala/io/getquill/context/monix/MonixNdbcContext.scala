...
object MonixNdbcContext {
  trait Runner {
    ...
    /** The schedule method can be used to change the scheduler that this task
      * should be run on
      */
    def schedule[T](t: Task[T]): Task[T]

    /** The schedule method can be used to change the scheduler that this
      * observable should be observed on
      */
  ...
  object Runner {
    ...
    def using(scheduler: Scheduler) = new Runner {
      override def schedule[T](t: Task[T]): Task[T] =
        t.executeOn(scheduler, forceAsync = true)

      override def schedule[T](o: Observable[T]): Observable[T] =
        o.executeOn(scheduler, forceAsync = true)
    }
  }

  object ContextEffect extends NdbcContextBase.ContextEffect[Task, Scheduler] {
    override def wrapAsync[T](f: (Complete[T]) => Unit): Task[T] =
      Task.deferFuture {
        val p = Promise[T]()
        f { complete =>
          p.complete(complete)
    ...
    override def fromDeferredFuture[T](f: Scheduler => Future[T]): Task[T] =
      Task.deferFutureAction(f(_))
...
/** Quill context that wraps all NDBC calls in `monix.eval.Task`.
  */
abstract class MonixNdbcContext[
    Dialect <: SqlIdiom,
  ...
  override private[getquill] val logger = ContextLogger(
    classOf[MonixNdbcContext[_, _, _, _]]
  )
  ...
  override implicit protected val resultEffect
      : NdbcContextBase.ContextEffect[Task, Scheduler] =
  ...
  override protected def withDataSource[T](
      f: DataSource[P, R] => Task[T]
  ): Task[T] =
    schedule(f(dataSource))

  protected def withDataSourceObservable[T](
      f: DataSource[P, R] => Observable[T]
  ): Observable[T] =
    schedule(f(dataSource))

  // TODO: What about fetchSize? Not really applicable here
  def streamQuery[T](
      fetchSize: Option[Index],
      sql: String,
      prepare: Prepare = identityPrepare,
      extractor: Extractor[T] = identityExtractor
  ...
  override private[getquill] def prepareParams(
    ...
    withDataSource { _ =>
      resultEffect.wrap(
        prepare(createPreparedStatement(statement), ())._1.reverse
          .map(prepareParam)
      )
    }
  }
