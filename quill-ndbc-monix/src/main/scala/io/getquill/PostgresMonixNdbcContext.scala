...
class PostgresMonixNdbcContext[N <: NamingStrategy](
  ...
  def this(naming: N, config: NdbcContextConfig, runner: Runner) =
    this(naming, PostgresDataSource.create(config.dataSource), runner)
  def this(naming: N, config: Config, runner: Runner) =
    this(naming, NdbcContextConfig(config), runner)
  def this(naming: N, configPrefix: String, runner: Runner) =
    this(naming, LoadConfig(configPrefix), runner)
  def this(naming: N, configPrefix: String) =
    this(naming, LoadConfig(configPrefix), Runner.default)
}
