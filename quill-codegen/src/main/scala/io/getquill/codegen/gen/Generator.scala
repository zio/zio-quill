...
trait Generator {
  ...
  type SingleGeneratorFactory[Emitter <: Generator#CodeEmitter] =
    ((EmitterSettings[TableMeta, ColumnMeta]) => Emitter)
  ...
  /** Should we prefix object/package produced by this generator? Set this as
    * the the value of that. Otherwise set this to be the empty string.
    */
  ...
  def generatorMaker = new SingleGeneratorFactory[CodeEmitter] {
    override def apply(
        emitterSettings: EmitterSettings[TableMeta, ColumnMeta]
    ): CodeEmitter =
      new CodeEmitter(emitterSettings)
  }

  class MultiGeneratorFactory[Emitter <: Generator#CodeEmitter](
      someGenMaker: SingleGeneratorFactory[Emitter]
  ) {
    def apply: Seq[Emitter] = {
      ...
      val emitters =
        genProcess.packageIntoEmitters(
          someGenMaker,
          packagingStrategy,
          stereotype(schemas)
        )
        ...
        val fileName: Path =
          (packagingStrategy.fileNamingStrategy, gen.codeWrapper) match {
            ...
            case (strategy: BySomeTableData[_], _)
                if (strategy.tt.tpe <:< scala.reflect.runtime.universe
                  .typeTag[Generator#CodeEmitter]
                  .tpe) =>
              strategy.asInstanceOf[BySomeTableData[CodeEmitter]].namer(gen)
          }

        val fileWithExtension =
          fileName.resolveSibling(fileName.getFileName.toString + ".scala")
  ...
  class CodeEmitter(emitterSettings: EmitterSettings[TableMeta, ColumnMeta])
    ...
    val caseClassTables: Seq[TableStereotype[TableMeta, ColumnMeta]] =
      emitterSettings.caseClassTables
    val querySchemaTables: Seq[TableStereotype[TableMeta, ColumnMeta]] =
      if (nameParser.generateQuerySchemas) emitterSettings.querySchemaTables
      else Seq()
    ...
    def caseClassesCode: String =
      caseClassTables.map(CaseClass(_).code).mkString("\n\n")
    def tableSchemasCode: String = querySchemaTables
      .map(CombinedTableSchemas(_, querySchemaNaming).code)
      .mkString("\n")
    ...
    class CaseClassGen(val tableColumns: TableStereotype[TableMeta, ColumnMeta])
      ...
      def code = {
        s"case class ${actualCaseClassName}(" + tableColumns.columns
          .map(Member(_).code)
          .mkString(", ") + ")"
      }
    ...
    class CombinedTableSchemasGen(
      ...
      override def objectName: Option[String] = Some(
        escape(tableColumns.table.name)
      )
      ...
      def body: String = {
        val schemas = tableColumns.table.meta
          .map(schema =>
            s"def ${querySchemaNaming(schema)} = " + indent(
              QuerySchema(tableColumns, schema).code
            )
          )
          .mkString("\n\n")
      ...
      class QuerySchemaGen(
          val tableColumns: TableStereotype[TableMeta, ColumnMeta],
          schema: TableMeta
