...
class StereotypePackager[Emitter, TableMeta, ColumnMeta] {
  ...
  def packageIntoEmitters(
    ...
    val caseClassDefs =
      groupByNamespace(tables, packageNamingStrategyForCaseClasses)
    // Generate (Packaging, [TableA, TableB, TableC]) objects for query schemas
    val querySchemaDefs =
      groupByNamespace(tables, packageNamingStrategyForQuerySchemas)
    // Merge the two
    val mergedDefinitions =
      caseClassDefs
        .zipOnKeys(querySchemaDefs)
        .map({ case (wrap, (caseClassOpt, querySchemaOpt)) =>
          (wrap, (caseClassOpt.toSeq, querySchemaOpt.toSeq))
        })
    ...
    groupingStrategy match {
      ...
      case DoNotGroup => {
        (caseClassTables ++ querySchemaTables)
          ...
          .map(_._1)
          .map(tc =>
            generatorMaker(EmitterSettings(Seq(tc), Seq(tc), wrapping))
          )
          .toSeq
      }
      case GroupByPackage =>
        Seq(
          generatorMaker(
            EmitterSettings(caseClassTables, querySchemaTables, wrapping)
          )
        )
    }
  }
