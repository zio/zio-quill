...
object PackagingStrategy {
  ...
  object ByPackageObject {
    private def packageByNamespace(prefix: String) =
      PackageObjectByNamespace(prefix, _.table.namespace)
  ...
  object ByPackageHeader {
    private def packageByNamespace(prefix: String) =
      PackageHeaderByNamespace(prefix, _.table.namespace)
    ...
    /** Use this strategy when you want a separate source code file (or string)
      ...
      * imports (i.e. since each file is a seperate table you can be sure to
      * just imports the exact tables needed for every source file).
      */
    ...
    /** When you want each file (or string) to contain an entire schema, use
      * this strategy. This is useful for code-generators that have common code
...
sealed trait PackageNamingStrategy
    extends (TableStereotype[_, _] => CodeWrapper)

object PackageNamingStrategy {
  type NamespaceMaker = TableStereotype[_, _] => String
}
case class PackageHeaderByNamespace(
    val prefix: String,
    val namespaceMaker: PackageNamingStrategy.NamespaceMaker
) extends PackageNamingStrategy
    with ByName {
  override def apply(table: TableStereotype[_, _]): CodeWrapper = PackageHeader(
    byName(table)
  )
}
case class PackageObjectByNamespace(
    val prefix: String,
    val namespaceMaker: PackageNamingStrategy.NamespaceMaker
) extends PackageNamingStrategy
    with ByName {
  override def apply(table: TableStereotype[_, _]): CodeWrapper = PackageObject(
    byName(table)
  )
}
case class SimpleObjectByNamespace(
    val prefix: String,
    val namespaceMaker: PackageNamingStrategy.NamespaceMaker
) extends PackageNamingStrategy
    with ByName {
  override def apply(table: TableStereotype[_, _]): CodeWrapper = SimpleObject(
    byName(table)
  )
}
