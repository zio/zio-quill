...
final case class PackageHeaderByNamespace(val prefix: String, val namespaceMaker: PackageNamingStrategy.NamespaceMaker)
    extends PackageNamingStrategy
    with ByName {
  override def apply(table: TableStereotype[_, _]): CodeWrapper = PackageHeader(byName(table))
}
final case class PackageObjectByNamespace(val prefix: String, val namespaceMaker: PackageNamingStrategy.NamespaceMaker)
    extends PackageNamingStrategy
    with ByName {
  override def apply(table: TableStereotype[_, _]): CodeWrapper = PackageObject(byName(table))
}
final case class SimpleObjectByNamespace(val prefix: String, val namespaceMaker: PackageNamingStrategy.NamespaceMaker)
    extends PackageNamingStrategy
    with ByName {
