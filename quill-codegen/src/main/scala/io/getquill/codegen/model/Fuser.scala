...
trait FuserBase[TableMeta, ColumnMeta] extends Fuser[TableMeta, ColumnMeta] {
  ...
  protected def unifyColumns(
      a: ColumnFusion[ColumnMeta],
      b: ColumnFusion[ColumnMeta]
  ) = {
...
class DefaultFuser[TableMeta, ColumnMeta](
    val ancestry: ClassAncestry = new CatalogBasedAncestry()
