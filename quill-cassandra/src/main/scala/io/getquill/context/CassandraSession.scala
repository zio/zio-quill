...
trait CassandraSession extends UdtValueLookup {
  ...
  val udtMetadata: Map[String, List[UserDefinedType]] =
    session.getMetadata.getKeyspaces.asScala.toList
      .map(_._2)
      .flatMap(_.getUserDefinedTypes.asScala.values)
      .groupBy(_.getName.toString)

  override def udtValueOf(
      udtName: String,
    ...
    udtMetadata.getOrElse(udtName.toLowerCase, Nil) match {
      ...
      case udts =>
        udts
          .find(udt =>
            keyspace.contains(
              udt.getKeyspace.toString
            ) || udt.getKeyspace.toString == session.getKeyspace.get().toString
          )
          .map(_.newValue())
          .getOrElse(
            fail(
