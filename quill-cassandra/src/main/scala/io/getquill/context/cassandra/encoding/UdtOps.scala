...
class UdtValueOps(val udt: UdtValue) extends AnyVal {
  def getScalaList[A](name: String, cls: Class[A]): mutable.Buffer[A] =
  ...
  def setScalaSet[A](name: String, v: Set[A], cls: Class[A]): UdtValue =
    udt.setSet(name, v.asJava, cls)

  def setScalaMap[K, V](name: String, v: Map[K, V], kcls: Class[K], vcls: Class[V]): UdtValue =
