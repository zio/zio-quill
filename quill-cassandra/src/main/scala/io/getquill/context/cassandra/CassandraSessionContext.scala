...
import io.getquill.context.cassandra.encoding.{
  CassandraTypes,
  Decoders,
  Encoders,
  UdtEncoding
...
trait CassandraPrepareContext[N <: NamingStrategy]
  ...
  protected def prepareAsync(cql: String)(implicit
  ...
  def probe(cql: String): Try[_] = {
    Try {
      Await.result(
        prepareAsync(cql)(ExecutionContext.Implicits.global),
        1.minute
      )
      ()
    }
  ...
  protected def prepareAsyncAndGetStatement(
      cql: String,
      prepare: Prepare,
      session: Session,
