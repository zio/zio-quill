...
class UdtEncodingMacro(val c: MacroContext) {
  ...
  private def UdtValueOps =
    q"_root_.io.getquill.context.cassandra.encoding.UdtValueOps"
  ...
  private def decodeUdt[T](udtType: Type) = {
    val t = udtFields(udtType).map {
      case (name, _, tpe, mapper, absType, absTypeDef, tag) =>
        ...
        def rawTree =
          q"$mapper.f(udt.get[$absType]($name, $classTree), session)"
        ...
        def single(tpe: Type) =
          q"$mapper: $encoding.CassandraMapper[$absType, $tpe]"
        ...
        val tree =
          if (tpe <:< typeOf[Option[Any]]) {
            ...
            params.appendAll(Seq(single(tpe.typeArgs.head), tagTree))
            val list =
              q"$UdtValueOps(udt).getScalaList[$absType]($name, $classTree)"
            q"$list.map(row => $mapper.f(row, session)).toList"

          } else if (isBaseType[Set[Any]](tpe)) {
            params.appendAll(Seq(single(tpe.typeArgs.head), tagTree))
            val set =
              q"$UdtValueOps(udt).getScalaSet[$absType]($name, $classTree)"
  ...
  private def encodeUdt[T](udtType: Type) = {
    ...
    val trees = ListBuffer[Tree](
      q"val udt = session.udtValueOf(meta.name, meta.keyspace)"
    )
    val (typeDefs, params) = udtFields(udtType).map {
      case (name, field, tpe, mapper, absType, absTypeDef, tag) =>
        ...
        if (tpe <:< typeOf[Option[Any]]) {
          ...
          val map =
            q"x.$field.map(kv => $mapper.f(kv._1, session) -> $vMapper.f(kv._2, session))"
          trees.append {
