...
trait Encoders extends CollectionEncoders {
  ...
  case class CassandraEncoder[T](encoder: BaseEncoder[T])
  ...
  implicit val bigDecimalEncoder: Encoder[BigDecimal] =
    encoder((index, value, row, _) =>
      row.setBigDecimal(index, value.bigDecimal)
    )
  ...
  implicit val byteArrayEncoder: Encoder[Array[Byte]] =
    encoder((index, value, row, _) =>
      row.setByteBuffer(index, ByteBuffer.wrap(value))
    )
  ...
  implicit val cassandraLocalTimeEncoder: Encoder[LocalTime] = encoder(
    _.setLocalTime
  )
  implicit val cassandraLocalDateEncoder: Encoder[LocalDate] = encoder(
    _.setLocalDate
  )
}
