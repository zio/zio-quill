...
class SessionBuilderSpec extends Spec {
  ...
  "creates Builder" - {
    ...
    "with a single host" in {
      val cfgString =
        """
          ...
          |""".stripMargin
      val sessionBuilder =
        SessionBuilder(ConfigFactory.parseString(cfgString).resolve())
      ...
      sessionConfig("basic.load-balancing-policy.local-datacenter") must equal(
        "dc1"
      )
      sessionConfig("basic.contact-points") must equal(
        new util.ArrayList(List(envAddress).asJava)
      )
      ...
      sessionConfig("advanced.connection.connect-timeout") must equal(
        "21 seconds"
      )
    }
    ...
    "with multiple hosts" in {
      ...
      sessionConfig("basic.contact-points") must equal(
        new util.ArrayList(List(envAddress, "127.0.0.2:29042").asJava)
      )
    }
    ...
    "with multiple hosts and keyspace" in {
      ...
      sessionConfig("basic.contact-points") must equal(
        new util.ArrayList(
          List(envAddress, "127.0.0.1:9042", "127.0.0.2:9042").asJava
        )
      )
    }
