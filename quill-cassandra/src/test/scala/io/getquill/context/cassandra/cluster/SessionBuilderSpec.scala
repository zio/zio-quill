  ...
  "creates Builder" - {
    ...
    "with a single host" in {
      val cfgString =
        """
          ...
          |}
          |""".stripMargin
      val sessionBuilder = SessionBuilder(ConfigFactory.parseString(cfgString))
      ...
      sessionConfig("basic.contact-points") must equal(new util.ArrayList(List("cassandra:9042").asJava))
    ...
    "with multiple hosts" in {
      val cfgString =
        """
          |{
          |   basic.contact-points = [ "cassandra:9042", "127.0.0.2:9042" ]
          |   basic.load-balancing-policy.local-datacenter = dc1
          |}
          |""".stripMargin
      val sessionBuilder = SessionBuilder(ConfigFactory.parseString(cfgString))
      ...
      session.getKeyspace.isPresent must equal(false)
      sessionConfig("basic.load-balancing-policy.local-datacenter") must equal("dc1")
      sessionConfig("basic.contact-points") must equal(new util.ArrayList(List("cassandra:9042", "127.0.0.2:9042").asJava))
    }
    ...
    "with multiple hosts and keyspace" in {
      val cfgString =
        """
          |{
          |   basic.contact-points = [ "cassandra:9042", "127.0.0.1:9042", "127.0.0.2:9042" ]
          |   basic.load-balancing-policy.local-datacenter = dc1
      ...
      sessionConfig("basic.load-balancing-policy.local-datacenter") must equal("dc1")
      sessionConfig("basic.contact-points") must equal(new util.ArrayList(List("cassandra:9042", "127.0.0.1:9042", "127.0.0.2:9042").asJava))
    }
